...
/**
 * The `/wall` route's own route entry, kept to a **lazy boundary only** (docs/plans/active/NGRX-MIGRATION-PLAN.md
 ...
 * `provideEffects` pair they need, therefore live in `wall.page-routes.ts` behind this `loadChildren`.
 *
...
export const WALL_ROUTES: Routes = [
  {
    path: 'wall',
    loadChildren: () => import('./wall.page-routes').then((m) => m.WALL_PAGE_ROUTES),
  },
];
