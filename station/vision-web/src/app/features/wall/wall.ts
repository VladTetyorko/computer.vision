 ...
 * (docs/plans/done/UI-ARCHITECTURE-PLAN.md): all orchestration (`FleetStore`/`SettingsStore`/`EventsStore`/
 * event-click navigation) lives in `WallFacade`, which this component injects exclusively.
 *
 * **Full-bleed + page bar** (docs/plans/done/NAV-IA-REDESIGN-PLAN.md §2.1/§2.2, docs/extracts/design/03-wall.md): the
 ...
 *
 * **`.surface-dark` enclave root** (docs/plans/done/VISUAL-REFRESH-PLAN.md F3/W4): `wall.html`'s outer `.page`
