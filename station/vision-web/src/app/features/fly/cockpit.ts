...
@Component({
  ...
  // `inject(DetectionsStore)`/`inject(WeatherFacade)`/`inject(SeatFacade)` resolve to these exact
  // instances (see `CockpitFacade`'s own doc comment).
  providers: [TelemetryStore, DetectionsStore, WeatherFacade, GeoFacade, SeatFacade, CockpitFacade, GroundingStore],
})
