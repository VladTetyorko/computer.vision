...
describe('FLY_ROUTES', () => {
  ...
  it('registers its thirteen page-scoped NgRx slices on the pathless parent, so picker → cockpit keeps one registration', async () => {
    const loaded = await (FLY_ROUTES[0].loadChildren as () => Promise<Routes>)();
    expect(loaded.map((route) => route.path)).toEqual(['']);
    expect(loaded[0].providers, 'the slices must be provided once, above both routes').toHaveLength(13);
  });
