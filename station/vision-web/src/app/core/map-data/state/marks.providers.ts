...
/**
 * The `marks` slice + its effects, registered by the routes that use it instead of at the root
 ...
 */
export function provideMarksState() {
  return makeEnvironmentProviders([provideState(marksFeature), provideEffects(marksEffects)]);
}
