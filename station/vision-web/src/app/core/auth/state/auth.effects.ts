...
export const reconnectLiveOnSession$ = createEffect(
  (actions$ = inject(Actions), liveStore = inject(LiveStore)) =>
    actions$.pipe(
      ofType(AuthApiActions.loginSucceeded, AuthApiActions.bootstrapSucceeded),
      tap(() => liveStore.reconnect()),
    ),
...
/**
 * Logout, stage 2 — the LiveStore stop + navigation `AuthStore#logout` used to do inline, now split
 * out because they're side effects the reducer can't own. Only stops LiveStore when there was a real
...
export const logoutSideEffects$ = createEffect(
  (actions$ = inject(Actions), router = inject(Router), liveStore = inject(LiveStore)) =>
    actions$.pipe(
