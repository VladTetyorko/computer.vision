...
import { REST_VALUE, knobLeftPercent, knobTopPercent, padsFrom, type ControlPad } from './control-surface-logic';
import { actionLabel, controlKey, controlLabel, positionOf, rulesFrom } from './control-action-logic';
...
function isControlBinding(binding: ManualControlChannelBinding | ControlBinding): binding is ControlBinding {
 ...
 */
export function toChannelBinding(binding: ControlBinding, catalog: ControlCatalog | undefined): ManualControlChannelBinding {
  return {
...
export function normalizeChannelMap(
  ...
  return channelMap.map((binding) => (isControlBinding(binding) ? toChannelBinding(binding, catalog) : binding));
}

/** The pads to draw — {@link normalizeChannelMap} then the unchanged `padsFrom`. */
export function displayPads(channelMap: ChannelMapLike, catalog: ControlCatalog | undefined): readonly ControlPad[] {
...
function restMark(
  ...
  return binding ? { percent: percentOf(binding, REST_VALUE), heavy: binding.travel === 'UNIDIRECTIONAL' } : undefined;
}
...
export function positionsForKind(kind: ControlInputKind, catalog: ControlCatalog | undefined): readonly SwitchPosition[] {
...
function readRaw(source: ControlSource, sourceIndex: number, axes: readonly number[], buttons: readonly number[]): number {
  ...
  return actionMap.map((binding) => {
    ...
    const cells = positions.map((p): ActionCell => {
      ...
      return {
        position: p,
        lit: p === position,
        text: fired ? actionLabel(fired.action, fired.parameter) : '—',
