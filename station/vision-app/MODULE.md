...
| `VisionPersistenceProperties` | moved; extended docs/plans/active/POSTGRES-ONLY-CONTEXT.md W1 with `seedDevUsers`, SCALE-100 S3 with `pool`, S4 with `telemetry` | `vision.persistence` | `adapter-persistence`'s `PersistenceUnit`, `TelemetryBatchSettings`; `vision-perception`'s `UsageSummaryBatchSettings` |
...
| `hlsProxySettings` | `com.drones.vision.api.support.VisionApiProperties.HlsProxy` | **docs/plans/active/SCALE-100-PLAN.md §5 S7**: `toApiSupportProperties(properties.hlsProxy())` — the second collaborator `HlsProxyController` now needs alongside `hlsProxyUpstreamBase`; see "HLS proxy / live-registry properties wiring" below |
| `liveSettings` | `com.drones.vision.api.support.VisionApiProperties.Live` | **docs/plans/active/SCALE-100-PLAN.md §5 S7**: `toApiSupportProperties(properties.live())` — the sixth collaborator `LiveUpdateRegistry` (vision-api, component-scanned) now needs; see "HLS proxy / live-registry properties wiring" below |
...
| `streamService` | `StreamService` | `ApplicationServiceWiring#streamService` builds `DefaultStreamService`'s **12-arg canonical ctor**: `deviceRepositoryPort, videoSourceRegistry, detectionPort, streamPublisherPort, detectionRepositoryPort, eventPublisherPort, usageTracker, detectionEventRepositoryPort, detectionLiveUpdatePort, streamPipelineSettings(...), pullDetectionSettings, detectionDemandPort`. **No `OverlayPort` parameter since docs/plans/active/CV-CLEAN-FEED-PLAN.md W1** (D-1: burn-in removed, not defaulted off) — before that wave `overlayPort` sat between `usageTracker` and `detectionEventRepositoryPort` (docs/plans/done/REALTIME-PLAN.md §4's original 10-arg shape, later widened to 12 by wave M7's `streamPipelineSettings`/`PullDetectionSettings` and then to 13 by docs/plans/active/CV-DEMAND-PLAN.md wave D2's `detectionDemandPort` — removing `overlayPort` brought the count back down to 12). `pullDetectionSettings` (**docs/plans/active/MEDIA-SOT-PLAN.md wave M7**) is `null` unless `VisionCvProperties#pullEnabled()`, built inline from `CvWiring`'s conditional `pulledDetectionPort` bean + `cvProperties.pull().rtspBase()`; `detectionDemandPort` (wave D2) is `null` unless `CvWiring#detectionDemandPort` resolved — see "CV inference wiring" and the wave D2 Status entry below. The bean method then wraps the result in `com.drones.vision.app.stream.LiveFrameFallbackStreamService` when `VisionPublishProperties.SourceProxy#enabled()` is `true` — see "Media source-of-truth wiring" below for both |
...
## Telemetry write batching (docs/plans/active/SCALE-100-PLAN.md S4)
...
## HLS proxy / live-registry properties wiring (docs/plans/active/SCALE-100-PLAN.md §5 S7)
...
## Overlay wiring — removed (docs/plans/active/CV-CLEAN-FEED-PLAN.md W1)
...
`detectionRepositoryPort` still backs the `GET /api/streams/{streamId}/detections` endpoint (`StreamController`, vision-api) exactly as before — always `JpaDetectionRepository` (docs/plans/done/MVP2-PLAN.md P-b) since docs/plans/active/POSTGRES-ONLY-CONTEXT.md W2b made it unconditional; that half of this section was never overlay-specific and needed no change.
...
## docs/plans/active/CV-CLEAN-FEED-PLAN.md W1 done (burn-in removed, deny-list filter added — vision-app half)
