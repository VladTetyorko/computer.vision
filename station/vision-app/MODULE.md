...
**Depends on:** vision-domain, vision-application, adapter-simulation, adapter-rtsp, adapter-mjpeg, adapter-mavlink, adapter-v4l2, adapter-publish-hls, adapter-cv-grpc, adapter-discovery, adapter-persistence, vision-api, vision-web (static-only jar), spring-boot-starter, **spring-boot-starter-security** (docs/plans/done/U-AUTH-PLAN.md wave 3 — Spring Security lives ONLY here, never in vision-api) · test: spring-boot-starter-test, archunit-junit5, **testcontainers-postgresql** (docs/plans/done/POSTGRES-ONLY-CONTEXT.md W4 — see "Test infrastructure" below). **`flyway-core` is a compile dependency, not a test one** (`3756a0a`) even though only testsupport code imports it directly: Maven's nearest-definition rule means a direct `test`-scoped declaration here overrides the transitive compile dependency from adapter-persistence, dropping `flyway-core` out of the repackaged jar entirely — `java -jar`, i.e. how `Dockerfile` runs it, then dies at boot with `NoClassDefFoundError: org/flywaydb/core/Flyway` while every test and IDE launch still passes
**Used by:** nothing (leaf/assembly module; produces the runnable jar via spring-boot-maven-plugin)
**Build/test:** `./mvnw -B -pl station/vision-app test` — green. Always `clean` first (see Gotchas). Requires the rest of the reactor already installed to the local repo (e.g. `./mvnw -B clean install -DskipTests -DskipWeb=true` once beforehand) since `-pl` alone doesn't build sibling modules from source. **Needs a reachable Docker daemon since docs/plans/done/POSTGRES-ONLY-CONTEXT.md W4** (every `@SpringBootTest` needs a real Postgres; since W2b, Postgres is not just the default but the *only* store — there is no `vision.persistence.enabled` flag left to turn it off) — see "Test infrastructure" below for what a Docker-less run does instead of failing.
...
com.drones.vision.app
  config/
    ...
    wiring/                per-concern @Configuration bean factories
      ...
      PublishWiring              mediamtx publish/replay, snapshotJpegEncoder,
                                 hlsProxyUpstreamBase, hlsProxySettings, liveSettings (SCALE-100 S7)
      ...
      SystemStatusWiring             new (docs/plans/done/SYSTEM-STATUS-PLAN.md wave S2): the four
  ...
  bootstrap/               SimulationResumeRunner (the one remaining ApplicationRunner —
                           AuthSeedRunner deleted docs/plans/done/POSTGRES-ONLY-CONTEXT.md W1, see
...
| `deviceRepositoryPort` | `DeviceRepositoryPort` | `PersistenceWiringConfiguration`: `new JpaDeviceRepository(entityManagerFactory)` (adapter-persistence) — unconditional, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b |
| `detectionRepositoryPort` | `DetectionRepositoryPort` | `PersistenceWiringConfiguration`: `new JpaDetectionRepository(entityManagerFactory)` (adapter-persistence) — unconditional, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b; append-only ring, capped per stream (docs/plans/done/MVP1-PLAN.md §C8 bullet 3) |
...
| `streamPublisherPort` | `StreamPublisherPort` | **docs/plans/done/MEDIA-SOT-PLAN.md wave M7**: `NoopStreamPublisher` (devsupport) if `vision.publish.enabled=false`, else a `PublisherRouter` (adapter-publish-hls) wrapping `MediamtxStreamPublisher` (unchanged 4-arg construction: `mediamtx.rtspBase()` to push, `VisionPublishProperties#viewBase()` — **not** `mediamtx.hlsBase()` — as `hlsViewBase`, `mediamtx.whepBase()` as `whepViewBase`, `mediamtx.playbackBase()` as `playbackViewBase`) as its direct publisher and `MediamtxProxyPublisher` (`mediamtx.apiBase()`, same view bases, `MediamtxProxySettings` mapped from `VisionPublishProperties.SourceProxy` + `mediamtx.apiUser()`/`apiPassword()`) as its proxy publisher, routed by `vision.publish.source-proxy.enabled` (default `false`, D1) — see "Media source-of-truth wiring" below |
| `mediamtxLiveFrameGrabber` | `MediamtxLiveFrameGrabber` | **docs/plans/done/MEDIA-SOT-PLAN.md wave M7**: `new MediamtxLiveFrameGrabber(mediamtx.rtspBase())` (adapter-publish-hls) — unconditional (cheap, no I/O at construction); consumed by `LiveFrameFallbackStreamService` when wired — see "Media source-of-truth wiring" below |
| `pulledDetectionPort` | `PulledDetectionPort` | **docs/plans/done/MEDIA-SOT-PLAN.md wave M7**: `CvWiring` — `GrpcPulledDetectionPort(cvGrpcChannel, toGrpcCvSettings(cvProperties))` present only when `VisionCvProperties#pullEnabled()` (`vision.cv.frame-transport=pull`); absent by default — see "Media source-of-truth wiring" below |
...
| `hlsProxySettings` | `com.drones.vision.api.support.VisionApiProperties.HlsProxy` | **docs/plans/done/SCALE-100-PLAN.md §5 S7**: `toApiSupportProperties(properties.hlsProxy())` — the second collaborator `HlsProxyController` now needs alongside `hlsProxyUpstreamBase`; see "HLS proxy / live-registry properties wiring" below |
| `liveSettings` | `com.drones.vision.api.support.VisionApiProperties.Live` | **docs/plans/done/SCALE-100-PLAN.md §5 S7**: `toApiSupportProperties(properties.live())` — the sixth collaborator `LiveUpdateRegistry` (vision-api, component-scanned) now needs; see "HLS proxy / live-registry properties wiring" below |
...
| `categoryRepositoryPort` | `CategoryRepositoryPort` | `PersistenceWiringConfiguration`: `new JpaCategoryRepository(entityManagerFactory)` (adapter-persistence) — unconditional, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b; seeding now lives in Flyway (`V1__*.sql`), not a devsupport ctor arg — see "Persistence wiring" below |
| `assetRepositoryPort` | `AssetRepositoryPort` | `PersistenceWiringConfiguration`: `new JpaAssetRepository(entityManagerFactory)` (adapter-persistence) — unconditional, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b |
| `assetUsageRepositoryPort` | `AssetUsageRepositoryPort` | `PersistenceWiringConfiguration`: `new JpaAssetUsageRepository(entityManagerFactory)` (adapter-persistence) — unconditional, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b |
| `assetImageRepositoryPort` | `AssetImageRepositoryPort` | `PersistenceWiringConfiguration`: `new JpaAssetImageRepository(entityManagerFactory)` (adapter-persistence) — unconditional, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b; docs/plans/done/UX-REWORK-PLAN.md §U-d item 3, CONTRACT 2's asset image store |
| `geofenceRepositoryPort` | `GeofenceRepositoryPort` | `PersistenceWiringConfiguration`: `new JpaGeofenceRepository(entityManagerFactory)` (adapter-persistence) — unconditional, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b; docs/plans/done/OPS-CORE-PLAN.md §G, G-b's geofence zone store |
| `telemetryRepositoryPort` | `TelemetryRepositoryPort` | `PersistenceWiringConfiguration`: `new JpaTelemetryRepository(entityManagerFactory)` (adapter-persistence) — unconditional, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b |
...
| `auditTrailPort` | `AuditTrailPort` | `new JpaAuditTrail(entityManagerFactory)` (adapter-persistence) — unconditional, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b, wrapped in `LiveUpdateAuditTrail` when `vision.live.enabled=true` (default) — see "Server-push data plane" below |
...
| `streamService` | `StreamService` | `ApplicationServiceWiring#streamService` builds `DefaultStreamService`'s **12-arg canonical ctor**: `deviceRepositoryPort, videoSourceRegistry, detectionPort, streamPublisherPort, detectionRepositoryPort, eventPublisherPort, usageTracker, detectionEventRepositoryPort, detectionLiveUpdatePort, streamPipelineSettings(...), pullDetectionSettings, detectionDemandPort`. **No `OverlayPort` parameter since docs/plans/done/CV-CLEAN-FEED-PLAN.md W1** (D-1: burn-in removed, not defaulted off) — before that wave `overlayPort` sat between `usageTracker` and `detectionEventRepositoryPort` (docs/plans/done/REALTIME-PLAN.md §4's original 10-arg shape, later widened to 12 by wave M7's `streamPipelineSettings`/`PullDetectionSettings` and then to 13 by docs/plans/done/CV-DEMAND-PLAN.md wave D2's `detectionDemandPort` — removing `overlayPort` brought the count back down to 12). `pullDetectionSettings` (**docs/plans/done/MEDIA-SOT-PLAN.md wave M7**) is `null` unless `VisionCvProperties#pullEnabled()`, built inline from `CvWiring`'s conditional `pulledDetectionPort` bean + `cvProperties.pull().rtspBase()`; `detectionDemandPort` (wave D2) is `null` unless `CvWiring#detectionDemandPort` resolved — see "CV inference wiring" and the wave D2 Status entry below. The bean method then wraps the result in `com.drones.vision.app.stream.LiveFrameFallbackStreamService` when `VisionPublishProperties.SourceProxy#enabled()` is `true` — see "Media source-of-truth wiring" below for both |
...
| `simulationService` | `SimulationService` | `new DefaultSimulationService(assetService, assetStreamService, deviceService, feedTransmitterRegistry, properties.mediamtx().rtspBase())` — docs/main/CYCLES-PLAN.md §1b, §3, §5's one-call, zero-hardware simulation entry point; builds on `assetService` (create) and `assetStreamService` (autoStart, W1.6e — see the `assetStreamService` row above) rather than the repositories directly, so it inherits every rule asset creation/streaming already enforces; `feedTransmitterRegistry` selects `rtspFeedTransmitter`/`mjpegFeedTransmitter` by protocol for `transport=rtsp`/`mjpeg` simulations; the mediamtx-base arg (backend follow-up batch) reuses `VisionPublishProperties.Mediamtx#rtspBase()` — no new property, same reuse as `rtspFeedTransmitter` above — so `DefaultSimulationService#resumeAll()` can recognize its own TX-fed feeds; see "Simulated-feed resume-on-boot" below. **3rd param `categoryRepositoryPort` → `deviceService` (docs/plans/active/ARCHITECTURE-AUDIT-2026-08-26.md R4)** — a net-zero-parameter swap: `deviceService` (an existing bean, already built elsewhere in this class) is what `SimulationService#fitSimulatedDevice` needs to register a fitted device; `categoryRepositoryPort` had no remaining use once `simulate()`'s category-seeded guard was removed (see `contexts/vision-simulation`'s own MODULE.md). Resolved by Spring's ordinary by-type parameter matching — no new bean, no explicit `@Qualifier` needed |
| `simulationResumeRunner` | `ApplicationRunner` | `new SimulationResumeRunner(simulationService, simulationProperties.resumeOnBoot())` — backend follow-up batch, simplified in docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b (persistence is unconditional now, so only `vision.simulation.resume-on-boot` gates it); see "Simulated-feed resume-on-boot" below |
...
`VisionPublishProperties` (`@ConfigurationProperties(prefix="vision.publish")`): `enabled` (`@DefaultValue("true")`), `viewBase: URI` (`@DefaultValue("/hls")`, defaulted again in the compact ctor if null — the URL base actually handed to viewers, app-relative by default so mediamtx's own address is never exposed to browsers), `mediamtx: Mediamtx` (defaulted as a whole in the compact ctor if null); nested `Mediamtx(rtspBase: URI, hlsBase: URI, whepBase: URI, playbackBase: URI, apiBase: URI, apiUser: String, apiPassword: String)` defaulting to `rtsp://localhost:8554` / `http://localhost:8888` / `http://localhost:8889` / `http://localhost:19996` / `http://localhost:19997` / unset / unset — **`hlsBase` is now purely the internal upstream `HlsProxyController` forwards to** (see `com.drones.vision.api.proxy.HlsProxyController`, vision-api), not a viewer-facing URL; that role moved to `viewBase`. **`whepBase` (docs/plans/done/MVP2-PLAN.md L-a) gets no such split** — `WiringConfiguration` hands `mediamtx.whepBase()` straight to `MediamtxStreamPublisher` as `whepViewBase` and it reaches the viewer unchanged, because a WHEP session (POST/SDP + ICE) cannot be reverse-proxied the trivial way HLS segments are; see "WHEP viewing (docs/plans/done/MVP2-PLAN.md L-a)" below. `apiBase`/`apiUser`/`apiPassword` (docs/plans/done/MEDIA-SOT-PLAN.md wave M7) are `MediamtxProxyPublisher`'s Control API endpoint/credentials — see "Media source-of-truth wiring" below. A sibling `SourceProxy` record (`enabled`/`onDemand`/`rtspTransport`/`readyTimeout`, all defaulted, `enabled` defaulting `false`) rides alongside `mediamtx` on this same properties record — same wave, same section.
...
## Telemetry write batching (docs/plans/done/SCALE-100-PLAN.md S4)

`vision.persistence.telemetry` is one block feeding two beans, because one arriving telemetry sample causes two writes:

| Bean | Class | Reads | Writes |
|---|---|---|---|
| `telemetryRepositoryPort` (`PersistenceWiringConfiguration`) | `JpaTelemetryRepository` | `toTelemetrySettings()` → `TelemetryBatchSettings` | the durable sample row |
| `usageTracker` (`ApplicationServiceWiring`) | `UsageTracker` | `toSummarySettings()` → `UsageSummaryBatchSettings` | the `AssetUsage` running-summary counter |
...
## Request rate limiting (docs/plans/done/SCALE-100-PLAN.md S6 item 3)
...
## HLS proxy / live-registry properties wiring (docs/plans/done/SCALE-100-PLAN.md §5 S7)

`PublishWiring#hlsProxySettings(VisionApiProperties)`/`#liveSettings(VisionApiProperties)` are two new unconditional `@Bean` methods, each mapping this module's Spring-bound `VisionApiProperties#hlsProxy()`/`#live()` onto the matching nested record of `com.drones.vision.api.support.VisionApiProperties` (vision-api's plain mirror) via the pre-existing `toApiSupportProperties` private helper `snapshotJpegEncoder` already used — same bridge shape, two more call sites. `HlsProxyController`'s `@Autowired` constructor now takes `com.drones.vision.api.support.VisionApiProperties.HlsProxy` as a second parameter alongside the pre-existing `hlsProxyUpstreamBase` `URI` bean; `LiveUpdateRegistry`'s now takes `com.drones.vision.api.support.VisionApiProperties.Live` as a sixth. Both beans are plain nested records, not the whole bridged top-level type — matching `snapshotJpegEncoder`'s own precedent of handing a controller only the slice of settings it actually declares.

Finishes what docs/plans/active/LAYERING-REFACTOR-PLAN.md Wave B/S1/S2/S5 each deferred: `HlsProxyController`'s `connectTimeout`/`requestTimeout`/`errorBodyPreviewMaxChars`/`maxRedirectHops` and `LiveUpdateRegistry`'s `coalesce`/`heartbeat`/`telemetryBuffer`/`eventBuffer`/`detectionBuffer`/`mapBuffer`/`sendTimeout`/`bufferEviction` were all `private static final` constants in vision-api until this wave; none of them are anymore. `vision.api.live.map-buffer` is the renamed `marks-buffer` key (the `marks`→`map` SSE topic rename, docs/plans/done/MAP-REWORK-PLAN.md §4.3, never propagated to this field name before now). **No `vision.api.live.dispatch-threads` key exists** — surveyed for this wave and deliberately not added: `LiveUpdateRegistry`'s per-connection write dispatch runs on `Executors.newVirtualThreadPerTaskExecutor()`, which has no pool-size/thread-count concept a knob by that name could honestly configure. See `station/vision-api/MODULE.md`'s own S7 wave entry and `LiveUpdateRegistry#connectionWriteExecutor`'s javadoc for the full reasoning; it applies only if that executor is ever swapped for a bounded platform `ThreadPoolExecutor`.

Every new key is commented out in `application.yaml`, documenting the pre-existing default rather than overriding it — this wave changes zero default-configuration behavior, confirmed by an unchanged `station/vision-app` test count: **195 → 195** (`./mvnw -B -pl storage/persistence,contexts/vision-perception,station/vision-api,station/vision-app -DskipWeb test`, Maven's own `Tests run:` summary line, Postgres/Testcontainers included — not skipped). The two new tests this wave added (`LiveUpdateRegistryTest#configuredTelemetryBufferCapacityActuallyBoundsTheRingBufferSize`, `HlsProxyControllerTest#configuredMaxRedirectHopsBoundsTheHandFollowedRedirectLoop`) both live in `vision-api`'s own suite (614 → 616), not this module's — see station/vision-api/MODULE.md's own S7 entry for that count and the sibling `storage/persistence`/`contexts/vision-perception` counts (157 → 157, 494 → 494), both untouched by this wave.

## Test infrastructure (docs/plans/done/POSTGRES-ONLY-CONTEXT.md W4 — a shared Postgres for `@SpringBootTest`)
...
## Controller setup wiring (docs/plans/active/CONTROLLER-SETUP-CONTEXT.md C6, wave C6)

`ControlProfileWiring` (`@Configuration`, `@EnableConfigurationProperties(VisionControlProperties.class)`) — two beans:

- `controlProfileService(ControlProfileRepositoryPort)` → `DefaultControlProfileService`, unconditional. Its repository comes from `PersistenceWiringConfiguration#controlProfileRepositoryPort` (`JpaControlProfileRepository`), like every other repository port in this app — Postgres is the only store.
- `auxFunctionCatalog(VisionControlProperties)` → the `RCx_OPTION` menu the setup page offers by name. Falls back to `AuxFunctionCatalog.defaults()` when `vision.control.aux-functions` is unset; **setting it replaces the menu rather than adding to it**. It is a *menu, not a whitelist* — `POST /api/assets/{id}/aux-function` accepts any number in `[0,400]` regardless — and everything else the setup page offers (input kinds, positions, functions, actions) comes from the domain enums and is deliberately not configurable, because that list is what this platform can genuinely command.

`ApplicationServiceWiring#manualControlService` gained a `ControlProfileService` parameter and passes `controlProfileService::activeFor` as `DefaultManualControlService`'s **7-arg** `ControlProfileSelector`. That method reference is the whole integration: engage now resolves the operator's own saved layout for the live vehicle kind, falling back to the built-in. Miss this wiring and there is no crash — the 6-arg constructor defaults to `ControlProfileSelector.builtInOnly()` and saved layouts silently do nothing (`contexts/vision-flight/MODULE.md` Gotchas).

`application.yaml` documents `vision.control.aux-functions` as a **commented-out** block showing the defaults rather than overriding them — the same "document the default in place" convention the `vision.rc.*` block already uses.
...
## Overlay wiring — removed (docs/plans/done/CV-CLEAN-FEED-PLAN.md W1)

The `PublishWiring#overlayRenderer()` bean (`Java2DOverlayRenderer`), `VisionOverlayProperties`, and the whole `adapter-overlay`/`video-output/overlay` Maven module described here since docs/plans/done/MVP1-PLAN.md §C8 are gone: **D-1** removed video burn-in entirely rather than defaulting it off. `DefaultStreamService` lost its `OverlayPort` constructor parameter (see the `streamService` bean-inventory row above) and `PublishWiring` lost the `overlayRenderer()` bean — no replacement bean, no flag. Boxes/labels/confidence are rendered client-side by the viewer now, never burned into the pixel stream; a deny-list (`labelDenyFilter`) lets an operator hide specific labels from that client-side render instead, enforced at the same single drop site `labelFilter` uses (`StreamPipeline#onDetectionResult`, contexts/vision-perception) — see contexts/vision-perception/MODULE.md and ARCHITECTURE.md's pipeline diagram for the rationale and the replacement shape.

`detectionRepositoryPort` still backs the `GET /api/streams/{streamId}/detections` endpoint (`StreamController`, vision-api) exactly as before — always `JpaDetectionRepository` (docs/plans/done/MVP2-PLAN.md P-b) since docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b made it unconditional; that half of this section was never overlay-specific and needed no change.
...
**Seeding, current mechanism (docs/plans/done/POSTGRES-ONLY-CONTEXT.md W1 replaced the `AuthSeedRunner`
...
## docs/plans/done/MEDIA-SOT-PLAN.md wave M7 done (media source-of-truth wiring — mediamtx as the video source of truth)
...
> (docs/plans/done/MEDIA-SOT-PLAN.md D4), so leaving vision.cv.frame-transport=push (its current
...
## docs/plans/done/CV-DEMAND-PLAN.md wave D2 done (system-derived detection demand — wiring + deployment properties)
...
## docs/plans/done/POSTGRES-ONLY-CONTEXT.md W1 done (the auth fix, standalone — `AuthSeedRunner` deleted)
...
## docs/plans/done/POSTGRES-ONLY-CONTEXT.md W4 done (Postgres becomes the default; the test suite gets a real database)
...
file to a single unconditional branch is explicitly a follow-up (docs/plans/done/POSTGRES-ONLY-CONTEXT.md
...
## docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b done (the deletion wave — flag removed, 19 `InMemory*` classes gone)
...
## docs/plans/done/SYSTEM-STATUS-PLAN.md wave S2 done (subsystem status endpoint — wiring half)
...
Postgres since docs/plans/done/POSTGRES-ONLY-CONTEXT.md W4; `OnboardingWiringTest` included.
...
**Docker**: ran, not skipped — every `@SpringBootTest` in this module needs a real Testcontainers
Postgres since docs/plans/done/POSTGRES-ONLY-CONTEXT.md W4.
...
## docs/plans/done/FIXED-CAMERA-GEO-PLAN.md Wave G4 done (fixed-camera geolocation, wiring half)
...
`@SpringBootTest` in the run (mandatory since docs/plans/done/POSTGRES-ONLY-CONTEXT.md W4).
...
## docs/plans/done/AFTER-ACTION-PLAN.md Wave W1 done (after-action evidence package, wiring half)
...
where applicable, per docs/plans/done/POSTGRES-ONLY-CONTEXT.md) — there is no new entity, no new
...
## docs/plans/done/VISUAL-GEO-V2-PLAN.md Wave H5 done (visual geolocation, wiring half)
...
**docs/plans/done/STREAM-STATE-PLAN.md wave S4 — wiring the idle-stream policy.**
...
## Wave H8 (docs/plans/done/VISUAL-GEO-V2-PLAN.md §9.11, §9.12) — done 2026-08-20
...
**R7 (docs/plans/active/ARCHITECTURE-AUDIT-2026-08-26.md R7, finding A2):** removed the three
remaining live-surface entries — `HlsProxyController#proxy` (now gated on `StreamAccess.requireVisible`),
`DeviceProbeController#probe` (annotated `@OpenByDesign`, no `AssetId`/`Ownership` in its request or
response to scope against), `EventController#forStream`/`#recent` (gated on `StreamAccess`/
`CurrentUser.scope()`). The ledger's remaining entries are all named `NOT in LIVE-SCOPE`, unowned by
this wave.

## docs/plans/done/CV-CLEAN-FEED-PLAN.md W1 done (burn-in removed, deny-list filter added — vision-app half)

**D-1** (burn-in is removed, not defaulted off) reaches this module as three separate deletions plus
the ordinary ripple of a `PipelineConfig`/`DefaultStreamService`/`StreamService` shape change made
upstream in `contexts/vision-perception` and `station/vision-api`:

1. **The `video-output/overlay` Maven module is gone entirely** — directory, `<module>overlay</module>`
   entry (`video-output/pom.xml`), and the root `pom.xml`'s `<dependencyManagement>` version pin all
   removed; `station/vision-app/pom.xml`'s own `adapter-overlay` `<dependency>` block removed
   alongside it. `grep -rn adapter-overlay **/pom.xml` across the whole reactor now returns nothing.
2. **`OverlayWiringTest`** (the class asserting the `OverlayPort` bean resolved to
   `Java2DOverlayRenderer`) deleted outright — its entire premise is now impossible, there is no
   `OverlayPort` bean, no `OverlayPort` type, nothing to assert.
3. **Every current-state section that named the deleted surface fixed in place**, not just left as
   stale narrative (this module's own "everything below Package shape is pre-existing content, left
   as written" convention applies to *historical Status entries*, not to sections a reader consults
   for today's wiring — see the "Overlay wiring" section immediately above, now retitled "removed"):
   the "Depends on" header line, the `PublishWiring` bullet in "Package shape", the
   `VisionOverlayProperties` row in the properties table, the `overlayRenderer` row in "Bean
   inventory", the `streamService` row (now documents the 12-arg canonical ctor with **no**
   `OverlayPort` parameter, and the arg-count history that produced today's 12: 10 (REALTIME §4) → 12
   (wave M7 added `streamPipelineSettings`/`PullDetectionSettings`) → 13 (CV-DEMAND wave D2 added
   `detectionDemandPort`) → 12 again (this wave removed `overlayPort`)), the `application.yaml`
   section's module-block list and wave-D new-keys sentence (both dropped `overlay:`/`vision.overlay.*`
   — the block itself was deleted from the actual `application.yaml` file), and the Test inventory's
   `OverlayWiringTest` bullet.
4. **Main-source fixes** for the `PipelineConfig`/`DefaultStreamService` shape change made upstream:
   `CvWiring#streamDefaultConfig` rebuilt against the new 9-arg `PipelineConfig` canonical constructor
   (drops `overlayTelemetry`/`overlayBurnIn`, adds `labelDenyFilter`);
   `LiveFrameFallbackStreamService` lost its now-illegal `burnedIn(StreamId)` override (`StreamService`
   no longer declares that method); `TrackProjectionRunner`'s doc comment ("latest raw (pre-overlay,
   full-resolution) frame" → "latest raw, full-resolution frame"). **The telemetry supplier remained
   unconditional throughout** — this module never gated it on anything overlay-related, so there was
   nothing to fix there.
5. **Test-source fixes**: `ArchitectureTest`'s adapter-slice enumeration comment dropped "overlay";
   `OnboardingWiringTest`'s dangling `{@link OverlayWiringTest}` (now deleted) repointed at
   `{@link CvWiringTest}` (same `@SpringBootTest(properties = "vision.publish.enabled=false")` shape);
   `CvDetectionE2ETest`/`TrackingAssociateE2ETest`/`CvDetectionEndpointE2ETest` all shared one stale
   `detectionEnabledDefaults()` helper calling the old 10-arg `PipelineConfig` constructor — all three
   rebuilt against the new 9-arg shape.

**Deviation, flagged rather than silently worked around**: `cv/grpc` (adapter-cv-grpc) is out of this
wave's file scope, and its *test* sources (`GrpcDetectionPortTest`, `GrpcPulledDetectionPortTest`) still
call the old 10-arg `PipelineConfig` constructor — its *main* sources compile cleanly against the new
9-arg shape, only test-compile is broken. This blocks a normal `-pl station/vision-app -am test` run.
Worked around without touching a single `cv/grpc` file: `./mvnw -B -pl station/vision-app -am install
-DskipWeb -Dmaven.test.skip=true` (installs every upstream module's *main* jar, `cv/grpc` included,
skipping test-compile reactor-wide) followed by `./mvnw -B -pl station/vision-app test` **without**
`-am`, so Maven resolves `cv/grpc` from the now-current `~/.m2` jar instead of re-test-compiling it in
the reactor. `cv/grpc`'s own test suite needs a follow-up fix by whichever agent owns that adapter —
not done here, out of scope.

**Build verification** (all foreground, none backgrounded): `contexts/vision-perception` **560/560**,
`station/vision-api` **753/753**, `station/vision-app` **242/242** (`./mvnw -B -pl station/vision-app
test`, from `clean`, via the two-step `-Dmaven.test.skip=true` install workaround above) — Docker was
reachable in this environment, so every `@SpringBootTest` ran against a real Postgres
(`testsupport.SharedPostgresContainer`) rather than being skipped.

**Other deviations from a literal reading of the plan, none silently improvised**:
- `DetectionExtrapolator#at()` (contexts/vision-perception) has zero production callers anywhere in
  the reactor after burn-in removal — its only consumer was the deleted overlay render path. Left in
  place (out of this wave's explicit file scope) and flagged as a Gotcha in
  contexts/vision-perception/MODULE.md, not deleted here.
- `latestFrame()`/`latestRawFrame()` stay two separate method names backed by one shared value in
...
## docs/plans/active/ARCHITECTURE-AUDIT-2026-08-26.md R4 done (device origin, wiring half)

`ApplicationServiceWiring#simulationService`'s 3rd constructor argument swapped from
`categoryRepositoryPort` to `deviceService` — see the `simulationService` bean-inventory row above
for the full rationale (a net-zero-parameter change: `DefaultSimulationService` no longer needs
`CategoryRepositoryPort` once its category-seeded guard was removed, and needs `DeviceService`
instead to back the new `fitSimulatedDevice`). `deviceService` was already an existing bean
(`ApplicationServiceWiring#deviceService`) — Spring's ordinary by-type `@Bean`-method-parameter
resolution picked it up with **zero additional wiring**. `SimulationResumeRunner`'s own doc-comment
paragraph updated to describe `resumeAll()`'s origin-based (not category-based) asset selection —
see the "Simulated-feed resume-on-boot" entry above.

No test in this module needed to change: `AssetWiringTest`, `SimulationResumeRunnerTest`,
`MjpegSimulationSmokeTest`, and `SimulationResumeWiringConfigurationTest` all reference
`SimulationService`/`simulationService` but none constructs `DefaultSimulationService` directly or
mocks the old `CategoryRepositoryPort` parameter — they all resolve the bean through the booted
Spring context, which the parameter swap does not disturb.

`./mvnw -B -pl station/vision-app -am test -DskipWeb`: **248/248 green** — the full
dependency-reactor build (every upstream module `station/vision-app` depends on, `adapter-mavlink`/
`adapter-cv-grpc`/`storage/persistence` included), confirming the wiring change is safe end to end.
Docker was reachable, so every `@SpringBootTest` ran against a real Postgres 16 container (V25
applied cleanly, confirmed in the boot log: "Successfully applied 25 migrations to schema \"public\",
now at version v25").
