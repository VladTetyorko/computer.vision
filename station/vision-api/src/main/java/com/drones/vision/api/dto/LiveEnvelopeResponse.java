 ...
 *                  fleet} (delivery narrows this list per connection to what its viewer may
 *                  currently see — {@code com.drones.vision.api.live.LiveConnection#project}),
 *                  {@code List<TelemetrySampleResponse>} for {@code telemetry} (a coalesced batch of
 *                  appended samples), a single {@code DetectionResultResponse} for {@code
 *                  detections} (latest-frame-only — no backlog), a single {@code EventResponse} for
 *                  {@code event}, a single {@code DevicesSnapshotResponse} for {@code devices}, a
 *                  single {@code DetectionEventResponse} for {@code detection-events}, and a single
 *                  {@link MapEventPayload} for {@code map} (docs/plans/done/MAP-REWORK-PLAN.md §4.3
 *                  — delivery of this one is filtered per connection by its own {@code layerId},
 *                  same mechanism as {@code fleet})
 */
