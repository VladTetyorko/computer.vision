...
| ControlProfileController | GET | `/api/control-profiles` | 200 `List<ControlProfileResponse>` — **the caller's saved layouts followed by the built-ins**, never empty; every row carries `stickMode`/`forwardIsUp` (C15), a built-in reporting `TransmitterView.DEFAULT` | — (docs/plans/active/CONTROLLER-SETUP-CONTEXT.md C6/C7; scoped to the acting user by ownership, not by `VisibilityScope`) |
...
| ControlProfileController | PUT | `/api/control-profiles/{id}` | 200 `ControlProfileResponse` | 400 bad id / invalid layout (a control bound twice, a channel driven twice, a `SWITCH_3` on a button, a missing action parameter — every one of them a domain `IllegalArgumentException`), 403 another operator's profile, 404 unknown id (body `UpdateControlProfileRequest{name, channelMap[], actionMap[], stickMode?, forwardIsUp?}` — the whole layout, replaced as a unit; the last two are C15's `TransmitterView`, **both optional in both directions** so an older client sends neither and gets the platform default, and a `stickMode` outside 1-4 is a 400) |
...
**Post-merge follow-up.** `ControlCatalogResponse` gained a trailing `maxRcChannel`, populated from `RcChannels.RELAYED_CHANNELS` (8). Decision C8's rule — the client invents nothing the server can state — applied to the one number it was still inventing: the setup page offered CH1–18 while the link relays CH1–8, so ten of those choices were stored, displayed, and never sent. 836 tests green.
