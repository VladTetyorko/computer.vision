 ...
 * CDK virtual scroll — see that component's own doc comment), and whatever `ui/fleet-map.ts`'s own
 ...
 *   `PollScheduler` idioms `pages/wall/wall-tile.ts` already established, not a new pattern.
 * - **Fleet map** — `<vision-fleet-map>` (`ui/fleet-map.ts`) reused **unmodified**, own
 ...
 * - **Events rail** — `<vision-events-rail>` (`ui/events-rail.ts`, extracted from `pages/wall/`
 ...
 * `@angular/cdk/scrolling` beyond `pages/devices/devices.ts`'s asset list (U2's own precedent,
