import { deriveTrail } from '../../core/telemetry-logic';
 ...
 * isolation, mirroring `core/telemetry-logic.ts`/`ui/player-recovery.ts`'s own split of pure
 * state/derivation from the component that drives it.
 *
 * `pages/replay/replay.ts` fetches a usage's timeline exactly once (`maxPoints: 2000`, the
 ...
 * chronological, gap-skipping shape `core/telemetry-logic.ts#deriveTrail` already produces for the
 ...
 * reading", not a gap) — `pages/replay/replay.ts` additionally checks `isDetectionNear` before
 ...
 * intermittently). `pages/replay/replay.ts`'s detections panel uses this to pick which of the two
