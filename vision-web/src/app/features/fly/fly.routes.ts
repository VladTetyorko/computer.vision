...
/**
 * The `/fly` route family (docs/MVP3-PLAN.md §C-b: the operator cockpit, the app's default landing
 * page). Split into its own file per vision-web/docs/UI-STRUCTURE-PLAN.md §2.3/§3 (B8) — every
 * feature owns its own route entry; `app.routes.ts` only composes them plus the shell-level
 * redirect/`**`.
 *
 * **Two routes now** (docs/NAV-IA-REDESIGN-PLAN.md §2.5 F12 — "the cockpit is not addressable"):
