 ...
 * itself — `ui/tile-cache-db.ts` is the thin, documented-but-untested glue around this.
 */

/**
 * One cache key per layer+zoom/x/y (docs/MVP3-PLAN.md: "per-layer keying incl. zoom") — the same
 * `{z}/{x}/{y}` a tile is fetched at is what the layer's `MapLayerId` (`core/settings-store.ts`)
 ...
 * `ui/tile-cache-db.ts` read from its lightweight metadata store (never the blob bytes themselves —
