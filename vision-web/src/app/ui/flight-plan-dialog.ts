 ...
 * action. Living in `ui/` rather than either `pages/devices/` or `pages/map/` follows the same
 * "no page imports another page's module" precedent as `ui/live-map.ts`/`core/device-logic.ts`
 * (see their own doc comments) — a shared `ui/` home instead of a cross-page import.
 *
 ...
 * user who'd rather type/paste than click. Reuses `ui/leaflet-loader.ts`'s dynamic import + the
