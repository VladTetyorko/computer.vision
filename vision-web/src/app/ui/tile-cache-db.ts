 ...
 * wiring — see `ui/live-map.ts`'s Leaflet bootstrap, `ui/player.ts`'s WHEP path — none of those have
 ...
 * surface `ui/leaflet-loader.ts`'s cached tile layer calls; every actual *decision* (the cache key,
 * which entries to evict) lives in the pure, unit-tested `ui/tile-cache-logic.ts` instead.
 *
