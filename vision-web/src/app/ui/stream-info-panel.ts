...
import { ageSeconds } from '../core/telemetry-logic';
import {
  describeSource,
  formatDuration,
  formatLatency,
 ...
 * `providers`, exactly like `TelemetryOsd`/`DetectionsStrip` already do; `pages/live/live.ts` and
 * `pages/asset-detail/asset-detail.ts` both do). `latencySeconds` is an input rather than a third
 * injected store because it isn't store-backed anywhere — it's `ui/player.ts`'s own measurement,
...
export class StreamInfoPanel {
  ...
  /** `ui/player.ts`'s own live transport, piped up via its `transportChanged` output (docs/MVP2-PLAN.md §L / §U3). */
