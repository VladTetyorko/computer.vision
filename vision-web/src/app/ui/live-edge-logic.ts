...
/**
 * Pure logic behind `ui/player.ts`'s live-edge behavior (docs/MVP2-PLAN.md §V, V-b): when a
 ...
 * timers, or a `<video>` element — mirrors `ui/player-recovery.ts`/`ui/detection-overlay-logic.ts`.
 *
 * Both functions here are deliberately transport/reducer-*aware*, not transport/reducer
 * *replacements* — `ui/player.ts` still drives every phase transition through
 ...
 *    `playing` (a stall or a fatal error triggered a reattach, and it succeeded). `ui/player.ts`
...
/**
 * Whether `ui/player.ts` should forward-seek the `<video>` to the live edge rather than let
 ...
 * for a barely-measurable drift. `null` (unmeasured, or the WHEP `0` pin — though `ui/player.ts`
