...
- **`SimulationService`** (interface) → **`DefaultSimulationService`** — the one-call, zero-hardware simulation entry point (docs/CYCLES-PLAN.md §1b, §3)
  - `DefaultSimulationService(AssetService, CategoryRepositoryPort, FeedTransmitterPort)` — builds one `AssetSpec` and delegates creation/streaming to `AssetService`, so every rule it already enforces (category validation, audit, device registration) applies here without duplication; `FeedTransmitterPort` backs `SimulationTransport.RTSP`
  - `SimulatedAsset simulate(SimulationSpec, Ownership, UserId actor)`:
    ...
    - Registers exactly two devices: a video device and a telemetry device (`"<name> · telemetry"`, protocol `"sim"`, a descriptive (never looked up) `sim://<slug>-telemetry` URI, `lat`/`lon` options only when provided in the spec, `Capability.TELEMETRY` — the telemetry device is what `SimulatedTelemetrySource` (adapter-simulation) opens for the circular home-point track). The video device depends on `spec.transport()`:
      ...
      - `RTSP` (docs/CYCLES-PLAN.md §3): a fresh `FeedId.random()`; `IllegalArgumentException` if `feedTransmitter.supports(new FeedSpec("rtsp", videoPath.toUri(), Map.of("loop","true")))` is `false`; otherwise `feedTransmitter.start(feedId, feedSpec)` and the video device is registered against the *returned* `StreamDescriptor`, augmented with `options.put("timeout","2000000")` (`StreamDescriptor` is immutable, so a new one is built) — the RX-side contention fix, see Gotchas.
    - `attributes` on the created `AssetSpec` includes `source` = the resolved absolute video path
    - `autoStart` → (for `RTSP`) a short fixed delay (see Gotchas), then `assetService.startStream(assetId, null, PipelineConfig.defaults())`; `SimulatedAsset.streamId()` is `null` otherwise
    - Tracks `AssetId -> FeedId` in a `ConcurrentHashMap` for every `RTSP`-transport asset (untracked for `DIRECT`); if `assetService.create`/`startStream` throws after a feed was started, the feed is stopped (and any partial tracking removed) before rethrowing — no orphaned transmitters
  - `void stop(AssetId)` (docs/CYCLES-PLAN.md §3): `assetService.stopStream(assetId)` always, then — if a feed is tracked for the asset — `feedTransmitter.stop(feedId)` and untrack. Idempotent; unknown/untracked/already-stopped asset is a no-op, mirroring `AssetService#stopStream`'s semantics
