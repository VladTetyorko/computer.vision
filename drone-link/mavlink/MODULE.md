...
**Post-merge follow-up.** `MavlinkManualControlSender`'s private `CHANNEL_COUNT` is now `RcChannels.RELAYED_CHANNELS` rather than its own literal 8 — the number of channels `RC_CHANNELS_OVERRIDE` carries is a fact about the relay that the API catalogue and the web picker also need, so it lives in the domain type that owns the frame. Behaviour unchanged; 192 tests green.
