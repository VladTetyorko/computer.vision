 ...
 * docs/plans/done/U-SCOPE-PLAN.md, U-e slice 2 — deferred slice-2 cleanup; {@link #create} migrated
 * onto {@link Authority}, wave B6).
 *
 * <p><strong>Management authority is derived from the acting {@link Authority}</strong> (wrapped
 * scope's kind maps 1:1 to role: unbounded = ADMIN, groups = MANAGER, else PILOT/empty). {@link
 * #create} takes the acting authority, {@link #list} the acting scope; {@link Authority#full()} /
 * {@link VisibilityScope#unbounded()} pass every gate — behavior is byte-identical to before these
 * gates existed.
 */
public interface GroupService {
     ...
     *
     * <p>{@code !acting.mayManageOrg()} → {@link AccessDeniedException}. A <strong>root</strong>
     ...
     *                                           and unknown (404)
     */
    Group create(GroupSpec spec, Authority acting);
