 ...
 *
 * <h2>Scope</h2>
 * {@link #start} requires {@link VisibilityScope#canManageOrg()} — any manager/admin
 ...
 * VisibilityScope#canManageOrg()} too — unlike the in-memory poll, the persisted run history is a
 * privileged administrative view (a deployment-wide record, not owned by any one dataset's group),
 * so lacking {@code canManageOrg()} means "may not view this at all", not "this run isn't yours" —
...
public interface TrainingJobService {
     ...
     * persisted run is tracked under — see {@link #run(TrainingRunId, UserId, VisibilityScope)}.
     *
     ...
     *         #run(TrainingRunId, UserId, VisibilityScope)}
     ...
     */
    List<TrainingRunRecord> runs(int limit, UserId actor, VisibilityScope scope);
     ...
     */
    TrainingRunRecord run(TrainingRunId runId, UserId actor, VisibilityScope scope);
}
