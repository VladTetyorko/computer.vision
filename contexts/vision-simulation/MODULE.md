...
- `vision-warehouse` — `Asset`/`Device`, `CategoryRepositoryPort`,
  `warehouse.application.asset.AssetService`/`AssetSpec`/`AssetSummary`,
  `warehouse.application.device.DeviceRegistration` (builds one `AssetSpec` and delegates
  creation/streaming to `AssetService`, so every rule it enforces — category validation, audit,
  device registration — applies here without duplication)
...
**Build/test:** `./mvnw -B -pl contexts/vision-simulation test` — **71/71 green** as of the W1.7b
...
- **`SimulationService`** (interface) → **`DefaultSimulationService`** — the one-call, zero-hardware
  ...
  - `DefaultSimulationService(AssetService assetService, AssetStreamService assetStreamService, CategoryRepositoryPort categoryRepository, FeedTransmitterRegistry feedTransmitters, URI mediamtxRtspBase)`
  ...
  - `SimulatedAsset simulate(SimulationSpec, Ownership, UserId actor)`:
    ...
    - Requires category `simulated` to exist in `CategoryRepositoryPort` (seeded by devsupport) — else
  ...
  - `List<AssetId> resumeAll()` — simulated-feed resume-on-boot: for every `simulated`-category, active
    asset not already tracked whose devices include an active `VIDEO`-capable, `rtsp`-protocol device
    whose `uri` host:port matches the injected `mediamtxRtspBase`, parses the trailing `feed-<uuid>`
