...
**docs/plans/active/DRONE-ONBOARDING-PLAN.md Wave O11 done** (making `VehicleProfileService#captureSnapshot` actually get called — see `contexts/vision-perception/MODULE.md` and `station/vision-app/MODULE.md` for the rest of this wave): added `PlatformActor` (own entry above), a one-file, purely additive change — no existing type's signature or behavior touched. `./mvnw -B -pl core/vision-platform -am test` — **17/17 green, unchanged** from before this wave (no new test file; see `PlatformActor`'s own entry for why).
