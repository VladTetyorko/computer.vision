...
- `record Authority(VisibilityScope scope, Set<Capability> capabilities)` (docs/plans/active/AUTH-ROLES-PLAN.md §3.1/§3.3, wave B1) — the authority axis, sibling to `VisibilityScope`'s visibility axis; wraps a `VisibilityScope` rather than copying or replacing it. `capabilities` defensively copied, `null`→empty. `boolean mayManageOrg()` — `Capability.MANAGE_ORG` held **and** `scope.canManageOrg()`; `boolean mayManageFleet(Ownership)` — `Capability.MANAGE_FLEET` held **and** `scope.canManage(ownership)`; `boolean mayAdminister()` — `Capability.MANAGE_ORG` held **and** `scope.canAdminister()`. Both halves of each `&&` must hold — a capability without a scope that reaches the resource (or vice versa) fails. `static Authority full()` — `VisibilityScope.unbounded()` + every `Capability` — the identity element: the dev principal, every `PrincipalResolver.fixed(...)`, and every existing test double get it, so every gate migrated onto `Authority` (wave B6) answers exactly as it did under a bare `VisibilityScope` — no new feature flag, `vision.auth.enabled` is still the only switch
