...
- `record GroupId(UUID value) implements Serializable` — `static random()`, `static of(String)`
...
- `record Ownership(UserId ownerId, GroupId groupId) implements Serializable`
...
- `enum UsageOrigin` — STREAM, OPERATOR. Which verb opened an `AssetUsage` (docs/plans/active/ARCHITECTURE-AUDIT-2026-08-26.md D2, wave R2) — mirrors `DeviceOrigin`'s shape. `STREAM`: a device's video stream starting (`UsageTracker#onStreamStarted`, the only opener before this wave). `OPERATOR`: the explicit `UsageTracker#engage`/`#disengage` verb — opens/closes a usage directly, no video stream, no device traffic. A third value, `TELEMETRY`, was considered and deliberately left out: R2 deleted `onTelemetryDeviceDiscovered` (the only code that would ever have produced it) as dead, so the value would name a session that cannot exist — add it back together with its producer, not before
- `record UsageId(UUID value)` — `static random()`, `static of(String)`
- `record UserId(UUID value) implements Serializable` — `static random()`, `static of(String)`
...
- **Only `UserId`/`GroupId`/`Ownership` implement `Serializable` today, not every kernel type** (AUTH-ROLES-PLAN.md B5-fix). These three are the only ones reachable from `station/vision-app`'s `VisionUserDetails` — the Spring Security principal that Spring Session JDBC java-serializes into `spring_session_attributes` once a session carries an authenticated user. Adding `implements Serializable` to a record costs nothing (`java.io` is JDK, not a third-party dependency — no conflict with "The rule" above) and is safe to do preemptively for any other kernel type a future principal/session-held object reaches through; it was done narrowly here rather than blanket-applied to all 17 types because nothing else in this module is ever handed to Java's serialization machinery, and an untested blanket change is not free (a `record` implementing `Serializable` commits to a stable field layout across releases in principle, even though nothing here relies on that yet).
...
gained an `origin` field of this type; see that module's `MODULE.md` for the persistence/API/
simulation follow-through. **205 → 205 tests** (the enum itself needs no new kernel-level test
beyond what `DeviceTest`/`DeviceRegistrationTest` in `vision-warehouse` already cover for its
call sites; `./mvnw -B -pl core/vision-kernel -am test`, green).

VISUAL-GEO-V2-PLAN wave **H2a** (docs/plans/done/VISUAL-GEO-V2-PLAN.md, 2026-08-19) added
...
FIXED-CAMERA-GEO wave **G1** (docs/plans/done/FIXED-CAMERA-GEO-PLAN.md, 2026-08-19) added
...
GEO-POSE-PLAN wave V1 (docs/plans/done/GEO-POSE-PLAN.md, 2026-08-15) added `Attitude` (new type),
...
AUTH-ROLES-PLAN B5-fix (2026-09-05, live-verification defect) — `UserId`, `GroupId`, `Ownership` each
gained `implements java.io.Serializable`. Root cause: `spring-boot-session-jdbc` (the correct Boot 4
artifact, replacing the inert bare `org.springframework.session:spring-session-jdbc` that shipped in
wave B5) java-serializes the whole `SecurityContext` — including `VisionUserDetails`'s `User` and
`Ownership` fields — into `spring_session_attributes` on every authenticated request. With the JDBC
store genuinely active, `POST /api/auth/login` 500'd with `NotSerializableException:
com.drones.vision.kernel.Ownership`; `AuthEnabledFlowTest` (station/vision-app) missed it because its
hand-built `MockMvc` never registers Spring Boot's real `SessionRepositoryFilter` (see that module's
own MODULE.md for the full explanation). No behavior change, no new validation — records only gained
an interface. **153 → 153 tests** (`./mvnw -B -pl core/vision-kernel,contexts/vision-identity test`,
green; this module's own suite is `UserId`/`GroupId`/`Ownership`-agnostic, so the count is unchanged).
