"""Model registry for ``cv_service.inference.detector.YoloDetector``: lazy
...
``inference/detector.py`` -- ``cv_service/grpc/servicers.py`` is the only
place that translates wire types (``FrameRequest.model_id``, the
``encoding`` enum) to/from the plain types used here. Importing this module
requires the ``cv`` extra (it imports ``cv_service.inference.detector``,
which imports ``cv2``/``numpy`` at module scope) -- callers only import it
lazily (see ``build_default_registry`` below and its callers in
``cv_service/grpc/server.py``/``servicers.py``).
"""
...
from cv_service.inference.detector import Detection, ModelUnavailableError, YoloDetector

LOGGER = logging.getLogger("cv_service.inference.registry")
...
class ModelRegistry:
    ...
    def resolve(self, requested_model_id: str) -> list[tuple[str, YoloDetector]]:
        ...
        - If every requested id was empty/unknown/unloadable (including a
          ...
          caller (`grpc/servicers.py`) echoes that frame. A partially-unknown
          composite list (e.g. `"orion12l.pt,typo"`) keeps its known
...
def detect_composite(
    ...
    *,
    ...
    - `inference_millis` returned is the SUM of every member's own
      ...
      sequentially, not overlapped (see MODULE.md for the measured
      per-model numbers this adds up from).
    - When more than one model actually ran, every detection's `label` is
      ...
      message has no dedicated model-tag field to carry this instead. A
      single-model request (the overwhelmingly common case) is completely
      unaffected: labels pass through exactly as the model named them.
...
def build_default_registry(settings: Settings) -> Optional[ModelRegistry]:
    """Build the production `ModelRegistry` from a resolved `Settings`.

    Assumes the `cv` extra is already available -- this module's own import
    already requires it (see module docstring), so by the time a caller has
    a reference to this function, `cv2`/`numpy`/`ultralytics` are known
    importable. Callers that don't yet know whether the extra is installed
    (`cv_service/grpc/server.py`'s `serve()`, `InferenceServicer`'s
    omitted-registry default path) import this module itself inside a
    `try/except ImportError`, mirroring this module's pre-split
    `_build_default_detector()`/`_build_default_registry()` contract exactly.

    `None` only when the *default* model itself can't be loaded (bad
    weights, corrupt file, etc.) -- the registry only changes what happens
    for *non-default* `model_id`s, never the "no model at all" story: the
    caller degrades the whole servicer to the Phase 0 echo behavior, same as
    always.
    """
    from cv_service.training.marker import read_active_model

    roster = discover_roster(settings.model_dir, settings.model)
    ...
    # (artifact removed) or no marker at all falls back to the resolved
    # model, never an error.
    promoted = read_active_model(settings.model_dir)
