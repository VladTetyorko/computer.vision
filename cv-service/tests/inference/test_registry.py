"""Unit tests for cv_service.inference.registry: discovery, routing, composite merge.

Needs the `cv` extra (imports `cv_service.inference.detector`, which needs
`cv2`/`numpy` at module scope -- same requirement as `test_detector.py`) but
...
from cv_service.inference.detector import Detection, ModelUnavailableError
from cv_service.inference.registry import ModelRegistry, detect_composite, discover_roster, short_name
...
def test_resolve_unknown_id_falls_back_to_default_and_warns_once(caplog):
    ...
    with caplog.at_level("INFO", logger="cv_service.inference.registry"):
...
def test_resolve_model_that_fails_to_load_falls_back_to_default(caplog):
    ...
    with caplog.at_level("WARNING", logger="cv_service.inference.registry"):
