...
# live (see cv_service/server.py's `_MODEL_SEARCH_DIR`, same directory).
_CV_SERVICE_DIR = Path(__file__).resolve().parent.parent
...
def test_registry_composite_real_weights_merges_and_prefixes_labels():
    ...
    from cv_service.inference import ENCODING_BGR24, ModelUnavailableError
    from cv_service.registry import ModelRegistry, detect_composite, discover_roster
