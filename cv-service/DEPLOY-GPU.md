...
cd vision/cv-service

python3.12 -m venv .venv
source .venv/bin/activate

# NOTE: no --extra-index-url here. cv-service/MODULE.md and pyproject.toml
...
gitignored — see `cv-service/MODULE.md` "`CV_MODEL` / weights location"):
...
`cv-service/` automatically (see `cv-service/MODULE.md` "Model registry &
...
3. **`inference_millis` drops vs. the CPU baselines.** `cv-service/MODULE.md`'s
