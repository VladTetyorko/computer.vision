...
public final class JpaGeofenceRepository implements GeofenceRepositoryPort {
    ...
    public GeofenceZone save(GeofenceZone zone) {
        GeofenceZoneEntity saved = jpa.write(em -> em.merge(toEntity(zone)));
    ...
    public Optional<GeofenceZone> findById(ZoneId id) {
        return jpa.read(em -> Optional.ofNullable(em.find(GeofenceZoneEntity.class, id.value())))
                .map(JpaGeofenceRepository::toDomain);
    }
    ...
    private static GeofenceZoneEntity toEntity(GeofenceZone zone) {
        return new GeofenceZoneEntity(zone.id().value(), zone.name(), zone.kind(), zone.polygon(),
                zone.maxAltitudeMeters(), zone.enabled());
    }

    private static GeofenceZone toDomain(GeofenceZoneEntity entity) {
