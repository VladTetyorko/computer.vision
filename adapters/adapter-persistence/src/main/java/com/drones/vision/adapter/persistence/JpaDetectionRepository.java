 ...
 *
 * <h2>Retention</h2>
 * docs/MVP2-PLAN.md P-b's retention guard, same mechanism as {@link JpaTelemetryRepository} but
 * keyed by {@code streamId} (the only grouping key {@link DetectionResult}/{@link DetectionQuery}
 * actually carry — there is no {@code usageId} on a detection): {@link #save} prunes a stream's
 * oldest results down to {@link #retentionLimitPerStream} whenever the count exceeds it, via one
 * native delete query per write in the same transaction as the insert, after an explicit {@code
 * flush()} for the same reason {@link JpaTelemetryRepository} needs one.
 */
public final class JpaDetectionRepository implements DetectionRepositoryPort {
     ...
     *                                JpaTelemetryRepository}'s two-argument constructor.
     */
    ...
    public List<DetectionResult> query(DetectionQuery query) {
        ...
        return candidates.stream()
                .map(JpaDetectionRepository::toDomain)
    ...
    private static DetectionResultEntity toEntity(DetectionResult result) {
    ...
    private static DetectionResult toDomain(DetectionResultEntity entity) {
