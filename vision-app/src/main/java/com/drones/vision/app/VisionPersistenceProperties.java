 ...
 *
 * <p>Selected by {@code PersistenceWiringConfiguration}: {@link #enabled()} {@code false} (the
 * default, today's behavior) keeps {@code CategoryRepositoryPort}/{@code DeviceRepositoryPort}/
 * {@code AssetRepositoryPort} wired to their devsupport in-memory fallbacks, so every existing
 * test and IDE run stays untouched; {@code true} wires {@code adapter-persistence}'s
 * {@code Jpa*Repository} implementations against {@link #jdbcUrl()}/{@link #username()}/
 * {@link #password()} instead, migrating the schema with Flyway on first use.
 *
