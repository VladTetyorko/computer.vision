...
| `simulationService` | `SimulationService` | `new DefaultSimulationService(assetService, categoryRepositoryPort)` — docs/CYCLES-PLAN.md §1b's one-call, zero-hardware simulation entry point; builds on `assetService` rather than the repositories directly, so it inherits every rule asset creation/streaming already enforces |
...
**Green.** Verified with a from-`clean` `./mvnw -B -pl vision-app test`: 16 tests, all passing — `VisionApplicationTests` 1, `DiscoveryDisabledWiringTest` 2, `FileSimulationSmokeTest` 1 (new, docs/CYCLES-PLAN.md §1c), `DiscoveryWiringTest` 1, `ArchitectureTest` 5, `SimStreamSmokeTest` 1, `AssetWiringTest` 1, `PublishWiringTest` 4.
