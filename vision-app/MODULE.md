...
- **ContextArchitectureTest** — 2 rules over `com.drones.vision.application..`, freezing the **bounded-context graph** (docs/plans/active/DOMAIN-SEPARATION-W1.md §4/W1.1). `crossContextEdgesMatchTheDeclaredSetExactly` asserts the observed set of cross-context dependencies **equals** `DECLARED_EDGES` — exact equality, so it fails both when a context reaches somewhere new *and* when a declared allowance goes stale, which is what forces the debt list to shrink instead of accumulating permanent exemptions. `acyclicContextsDoNotDependOnEachOtherInBothDirections` holds mutual pairs at zero, because Maven cannot express a cycle and one mutual pair would block extraction for every context at once. Contexts are declared as a package→context map in the test itself; `vision-domain` is deliberately out of scope until its packages exist (W1.5). Note that ArchUnit reads **bytecode**, so the ~98 javadoc-only `{@link}` imports measured in W1 do not register as edges here — they are still real work before Maven extraction, since an unused import must resolve at compile time (W1.4). One remaining declared debt: `warehouse -> perception` (C3)
