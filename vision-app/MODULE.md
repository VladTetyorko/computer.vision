...
- **ContextArchitectureTest** — 4 rules (grew from 2 in wave W1.5a, docs/plans/active/DOMAIN-SEPARATION-W1.md §4/W1.1/W1.5a), freezing the **bounded-context graph** on both the application and the (now package-walled) domain layer. `importPackages` widened from `com.drones.vision.application` to `com.drones.vision` so the two new domain-facing rules below have something to scan.
  - `crossContextEdgesMatchTheDeclaredSetExactly` (application layer, unchanged since W1.2) asserts the observed set of cross-context dependencies **equals** `DECLARED_EDGES` — exact equality, so it fails both when a context reaches somewhere new *and* when a declared allowance goes stale, which is what forces the debt list to shrink instead of accumulating permanent exemptions. One remaining declared debt: `warehouse -> perception` (C3).
  - `acyclicContextsDoNotDependOnEachOtherInBothDirections` holds mutual pairs in `DECLARED_EDGES` at zero, because Maven cannot express a cycle and one mutual pair would block extraction for every context at once.
  - `domainCrossContextReferencesMatchTheDeclaredSetExactly` (new — the domain layer had no package boundary to enforce against until W1.5a moved `domain.model`/`domain.port.out` into `com.drones.vision.<context>.domain.model`/`.domain.port`) — the same exact-match contract as the application rule above, over `DECLARED_DOMAIN_EDGES`: `events -> flight`, `events -> map`, `events -> perception`, `flight -> warehouse`, `learning -> perception`, `perception -> flight`, `perception -> warehouse` — acyclic, no declared debt (C4/C5 already broke the domain layer's one cycle, `perception <-> events`, as an assignment decision in W1.2; W1.5a is what made it a real package boundary).
  - `kernelDependsOnNothingButItselfAndTheJdk` (new) — the shared kernel (`com.drones.vision.kernel`, 15 types: every id, `GeoPosition`, `Ownership`, `LifecycleState`, `Capability`, `BoundingBox`, `BearingDistance`, `GeoProjection`, `StreamDescriptor`) must depend on no context's domain code, because a kernel that did would smuggle that one context's edge into all eight contexts that are allowed to depend on the kernel.

  Contexts are declared as a package→context map in the test itself. ArchUnit reads **bytecode**, so the ~98 javadoc-only `{@link}` imports measured in W1 never registered as edges here — that cleanup (W1.4) was still real work before Maven extraction, since an unused import must resolve at compile time.
...
2. **Optional burn-in skip** — **no new `vision-app` property was added.** The task brief asked to add one "if such a pattern exists (follow precedent; if none, `PipelineConfig` default + API override suffices — document)": no existing `vision-app` property overrides any other `PipelineConfig.defaults()` field either (`confidenceThreshold`/`inferenceFps`/`labelFilter`/`overlayTelemetry`/`eventRule` are all API-override-only today, nothing global-configurable at the `vision-app` layer) — `overlayBurnIn` follows that same precedent exactly rather than inventing a new one, so the whole feature is domain default (`true`) + `vision-api`'s new `StartStreamRequest#overlayBurnIn`/`StartAssetStreamRequest#overlayBurnIn` per-stream override (see vision-api/MODULE.md). No `WiringConfiguration`/`application.yaml` change was needed or made — `PipelineConfig` already flows through this module's wiring as an opaque per-call argument (`DefaultStreamService#start(DeviceId, PipelineConfig)`), so the new field needed no plumbing here at all, the same way `eventRule` (E-a) needed none either.
