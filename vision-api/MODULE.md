...
**Green**, verified by a from-`clean` `./mvnw -B -pl vision-api clean test`: main and test sources compile with zero errors, 83/83 tests pass (AssetControllerTest 21, StreamControllerTest 13, DiscoveryControllerTest 8, DeviceControllerTest 10, CategoryControllerTest 2, HlsProxyControllerTest 6, SimulationControllerTest 23 — added for docs/CYCLES-PLAN.md §1c's one-call simulation endpoint, extended for §3's `transport` field and `DELETE /api/simulations/{assetId}`, for §5's `"mjpeg"` transport value, and for §7's `telemetry` flight-plan object).
