...
| SimulationController | POST | `/api/simulations` | 201 `SimulationResponse` | 400 bad/nonexistent `videoPath`, 409 `simulated` category not seeded |
...
**Green**, verified by a from-`clean` `./mvnw -B -pl vision-api clean test`: main and test sources compile with zero errors, 69/69 tests pass (AssetControllerTest 21, StreamControllerTest 13, DiscoveryControllerTest 8, DeviceControllerTest 10, CategoryControllerTest 2, HlsProxyControllerTest 6, SimulationControllerTest 9 — the last added for docs/CYCLES-PLAN.md §1c's one-call simulation endpoint).
