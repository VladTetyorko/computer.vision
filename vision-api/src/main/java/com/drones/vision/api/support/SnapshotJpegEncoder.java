...
public final class SnapshotJpegEncoder {
     ...
     */
    public static final int MAX_SNAPSHOT_WIDTH = VisionApiProperties.Snapshot.defaults().maxWidth();

    private final int maxWidth;
    private final float jpegQuality;

    public SnapshotJpegEncoder(VisionApiProperties properties) {
        Objects.requireNonNull(properties, "properties must not be null");
        this.maxWidth = properties.snapshot().maxWidth();
        this.jpegQuality = properties.snapshot().jpegQuality();
    }

    /**
     * Encodes {@code frame} to JPEG bytes, downscaled to at most {@link #maxWidth}px wide. A frame
     * that is already {@link PixelFormat#JPEG} and already at or under that width is returned
     * untouched (no decode/re-encode round trip) — the common case for
    ...
    public byte[] encode(VideoFrame frame) {
        if (frame.format() == PixelFormat.JPEG && frame.width() <= maxWidth) {
    ...
    private BufferedImage decode(VideoFrame frame) {
    ...
    private BufferedImage downscale(BufferedImage source) {
        ...
        BufferedImage scaled = new BufferedImage(maxWidth, scaledHeight, BufferedImage.TYPE_3BYTE_BGR);
        Graphics2D g = scaled.createGraphics();
        try {
            g.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
            g.drawImage(source, 0, 0, maxWidth, scaledHeight, null);
        } finally {
            g.dispose();
        }
        return scaled;
    ...
    /** Encodes {@code image} as a JPEG at {@link #jpegQuality} quality via an explicit ImageWriter. */
