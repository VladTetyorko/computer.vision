 ...
 *
 * <p>Package-private, stateless, no interface ({@code .claude/skills/java-clean-code/SKILL.md}
 * §1) — {@link SystemNetworkController}'s one collaborator, with no second implementation ever
 * plausible; the same "small package-private helper, not a domain/application port" shape {@code
 * SnapshotJpegEncoder} already establishes for this module's own byte-level infrastructure. The
 * {@link NetworkInterface} enumeration itself sits behind an injectable {@link
 * NetworkInterfaceSource} (the package-private second constructor) purely so a unit test can feed
 * a fixed, fake interface list — including one that throws {@link SocketException} — instead of
 * depending on whatever NICs happen to be present on the machine running the build.
 */
final class LocalNetworkAddresses {
     ...
     */
    List<NetworkAddressResponse> list() {
