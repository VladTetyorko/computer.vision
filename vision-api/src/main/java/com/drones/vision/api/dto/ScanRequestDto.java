 ...
 * the corresponding value from {@link DiscoveryScanSpec#defaults()} — a 4-second
 ...
 * @param methods   which mechanisms to run, by {@code DeviceDiscoveryPort#method()} key; {@code null} uses the default (every registered mechanism); an explicit empty set also means "every registered mechanism" (see {@link DiscoveryScanSpec})
 */
public record ScanRequestDto(Long timeoutMs, Set<String> methods) {

    /** No body: use every default from {@link DiscoveryScanSpec#defaults()}. */
    public static final ScanRequestDto EMPTY = new ScanRequestDto(null, null);

    /**
     * Converts this request into a {@link DiscoveryScanSpec}, applying defaults
     ...
     */
    public DiscoveryScanSpec toScanRequest() {
        DiscoveryScanSpec defaults = DiscoveryScanSpec.defaults();
        ...
        return new DiscoveryScanSpec(timeout, effectiveMethods);
    }
}
