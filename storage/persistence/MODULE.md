...
detections leave behind (docs/plans/done/FIXED-CAMERA-GEO-PLAN.md wave G3) — and, for visual
geolocation, the corrected track a moving asset's camera feed yields once matched against a reference
tile index (docs/plans/done/VISUAL-GEO-V2-PLAN.md wave H5).

**Depends on:** vision-domain, `org.hibernate.orm:hibernate-core`, `org.hibernate.orm:hibernate-hikaricp`
(docs/plans/done/SCALE-100-PLAN.md S3 — declared for its version pin only, see "Connection pool" below for why
...
`CameraPoseRepositoryPort`/`TrackTrailRepositoryPort` (`contexts/vision-map`, wave G2), `V22__fixed_camera_geo.sql`
— +14 `CameraPoseRepositoryTests`/`TrackTrailRepositoryTests` round-trip cases, +2 `DbAuditLogRepositoryTests`
cases (a pose update names its changed column, a trail insert leaves no row), +1 `v22Migration...` schema
test, both `DbAuditLogCoverageTests` lists updated; see "Database change audit" and this module's own
entity/repository bullets below for the full account) — 148 tests (up from 137 — measured directly
via `./mvnw -B -pl storage/persistence clean test`, docker reachable, nothing skipped; docs/plans/done/SCALE-100-PLAN.md
...
docs/plans/done/POSTGRES-ONLY-CONTEXT.md **upgrade path**: this wave fixes the path for a database
...
docs/plans/done/POSTGRES-ONLY-CONTEXT.md W3 — the two ports that previously had no Postgres
...
`JpaTrackCorrectionRepository`; up from 22 — docs/plans/done/FIXED-CAMERA-GEO-PLAN.md wave G3
...
`FeatureRequirementMapper`, `CameraPoseMapper`, `TrackPointMapper` — 22 mappers (up from 20,
docs/plans/done/FIXED-CAMERA-GEO-PLAN.md wave G3), one per aggregate the module's 23 repositories
cover. `CameraPoseMapper#toEntity`/`TrackPointMapper#toEntity` each flatten their record's single
`GeoPosition position` field to `latitude`/`longitude`/nullable `altitudeMeters` columns — the same
choice `MarkMapper` already makes for its own single-point position — and take no extra id argument
(`CameraPose`/`TrackPoint` both already carry every id their entity needs: `assetId`, and for
`TrackPoint`, `trackId`). `TelemetryMapper#toEntity`/`DetectionResultMapper#toEntity`
...
- `DbAuditLogEntity` (`V21__db_audit_log.sql`) — the database change audit; see "Database change audit" below for the full picture. **Read-only from Hibernate's side**: every row is written by that migration's `audit_row_change()` trigger function, never by a `persist`/`merge` call anywhere in this module — mapped here purely so `JpaDbAuditLogRepository`'s reads go through the same `jakarta.persistence` API as every other repository. `id` is a synthetic `BIGINT GENERATED ALWAYS AS IDENTITY` column (`@GeneratedValue(strategy = GenerationType.IDENTITY)`) rather than a UUID this module invents in Java — the only entity in this schema whose id Postgres itself generates, since the trigger that creates each row runs entirely inside the database. `rowId` is `String`, not `UUID`, even though every primary key in this schema is in fact a UUID (or, for the composite-key join tables, several) — the trigger resolves it generically from the catalog and cannot assume one uniform key shape across all eighteen audited tables (seventeen as of V21, `camera_poses` added by V22 — see "Database change audit" below). `oldRow`/`newRow` map to `Map<String, Object>` and `changedColumns` to `List<String>`, both jsonb — unlike every other jsonb column in this module (which always has one fixed domain record/map shape on the far side), these have no fixed shape at all, since the row image can belong to any of those eighteen audited tables. `oldRow` is `null` for an `INSERT`, `newRow` is `null` for a `DELETE`, `changedColumns` is `null` (not an empty list) for anything but an `UPDATE`. `operation` reuses a small persistence-local enum, `DbAuditOperation` (`INSERT`/`UPDATE`/`DELETE`, mirroring Postgres's own `TG_OP`) — not a domain enum, since there is no domain concept of "a database row changed."
- `CameraPoseEntity` (docs/plans/done/FIXED-CAMERA-GEO-PLAN.md decision D4, wave G3, `V22__fixed_camera_geo.sql`) mirrors `CameraPose` field-for-field: `assetId` is the primary key, not synthetic — one calibrated pose per fixed asset, `save` is always an upsert, same "owning id doubles as the uniqueness constraint" shape as `AssetImageEntity`. `position` (a single `GeoPosition`) is flattened to `latitude`/`longitude`/nullable `altitude_meters` columns, same choice `MarkEntity` makes for its own single point — altitude is currently unused by the projection math but the column exists anyway so a round trip cannot silently drop it (a gap the plan's own §7 table sketch left out; flagged rather than silently worked around). `source` reuses the domain `CameraPoseSource` enum directly (`@Enumerated(EnumType.STRING)`), same convention as `MarkEntity#source`. `targetLayerId`/`rmsErrorPixels` are nullable. No FK to any other table. **Audited** (`trg_audit_camera_poses`) — see "Database change audit" below.
- `TrackPointEntity` (docs/plans/done/FIXED-CAMERA-GEO-PLAN.md decision D3/§7, wave G3, `V22__fixed_camera_geo.sql`) mirrors `TrackPoint` field-for-field: `id` is a synthetic `BIGINT GENERATED ALWAYS AS IDENTITY` column the schema itself generates (same mechanism as `DbAuditLogEntity#id`, not invented in Java) — `TrackPoint` carries no id of its own, a decimated append-only breadcrumb never looked up by id, only by `(assetId, trackId)` or pruned by `capturedAt`. `position` is flattened to `latitude`/`longitude`/nullable `altitude_meters`, same choice as `CameraPoseEntity` (same §7 gap, same fix). Indexed on `(asset_id, track_id, captured_at)` (the `findByTrack`/`trimToMostRecent` access path) and `captured_at` alone (`deleteOlderThan`'s access path). No FK to any other table. **Excluded** from `db_audit_log` — high-volume machine output, the opposite of `CameraPoseEntity` — see "Database change audit" below.
- `ControlProfileEntity` (docs/plans/active/CONTROLLER-SETUP-CONTEXT.md C6, wave C4, `V24__control_profiles.sql`) — `id` UUID PK (the domain's own `ControlProfileId`, not synthetic); `ownerUserId`, `vehicleKind`/`code`/`displayName` as strings, `active`, `updatedAt`, plus `stickMode`/`forwardIsUp` (`V25`, C15 — the owner's `TransmitterView`, two scalar columns rather than a third `jsonb` blob: they are two fixed fields with a range check, not an open document). `channelMap`/`actionMap` are **`jsonb` columns holding the domain records directly** (`List<ControlBinding>`/`List<ActionBinding>` through the module's existing Jackson `@JdbcTypeCode(SqlTypes.JSON)` convention, the same shape `VehicleProfileEntity`/`DetectionResultEntity` use) — a profile is only ever read back whole, never queried into by individual binding, so a normalized child table would buy nothing and cost two joins. No FK to `users` (this schema's standing convention). **Audited** (`trg_audit_control_profiles`).
...
- `V2__seed_categories.sql` — the default category set (`drone`, `ip-camera`, `usb-camera`, `robot`, `simulated`, then `fpv-drone`→`drone`, `esp32-cam`→`ip-camera` in a second batch so the self-referencing FK is satisfied), `ON CONFLICT (id) DO NOTHING` so re-running is a no-op — the same set the deleted `InMemoryCategoryRepository` (vision-app devsupport, removed docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b) used to seed at construction, kept identical so removing the in-memory fallback changed no out-of-the-box category list.
...
- `V13__identity_baseline.sql` (docs/plans/done/POSTGRES-ONLY-CONTEXT.md W1) — seeds the root group at the
...
- `V25__control_profile_transmitter_view.sql` (docs/plans/active/CONTROLLER-SETUP-CONTEXT.md C15) — two columns on `control_profiles`, purely additive: `stick_mode SMALLINT NOT NULL DEFAULT 2` and `forward_is_up BOOLEAN NOT NULL DEFAULT TRUE`, plus `ck_control_profiles_stick_mode CHECK (stick_mode BETWEEN 1 AND 4)`. Defaults rather than nullables because every existing row *does* have an arrangement — the platform's — and a nullable column would push "unknown" into a domain record that has no such state. The check duplicates `TransmitterView`'s own compact-constructor range on purpose: this table is written by JPA today and by a migration or a fix-up script tomorrow.

### `src/main/resources/db/seed/dev` — a second, conditional Flyway location

- `V90001__dev_accounts.sql` (docs/plans/done/POSTGRES-ONLY-CONTEXT.md W1) — the DEV-ONLY
  ...
  **Guard shape (docs/plans/done/POSTGRES-ONLY-CONTEXT.md upgrade path, defect 1):** each account is
...
UUID (CLAUDE.md rule 1: no per-table logic duplicated eighteen times where one generic rule already
...
| `camera_poses` | V22 | one calibrated pose per fixed asset — control-plane, low-write-volume, the same "one row per key" character as `vehicle_profiles`'s own newest-row query, but mutated in place (upsert) rather than append-only |
...
`vision.persistence.pool.*` (naming matches docs/plans/done/SCALE-100-PLAN.md §6) and constructs the record:
...
## Batching (docs/plans/done/SCALE-100-PLAN.md S4)
...
- `@Nested TelemetryRepositoryTests` (10, up from 7 — docs/plans/done/FC-INTEGRATIONS-PLAN.md F-b) — round trip
  ...
  *earliest* samples (mirroring `InMemoryTelemetryRepository`'s actual behavior, see
  ...
  would also satisfy — plus two new docs/plans/done/SCALE-100-PLAN.md S4 batching cases:
...
- `@Nested AuditTrailRepositoryTests` (4, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W3) — a recorded entry
...
- `@Nested DetectionEventRepositoryTests` (7, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W3) — an open event
...
- `v14MigrationCreatesTheAuditEntriesTableOnTopOfV1ThroughV13` (docs/plans/done/POSTGRES-ONLY-CONTEXT.md W3) —
...
- `v15MigrationCreatesTheDetectionEventsTableOnTopOfV1ThroughV14` (docs/plans/done/POSTGRES-ONLY-CONTEXT.md
...
`@Nested DbAuditLogRepositoryTests` (5, up from 3 — docs/plans/done/FIXED-CAMERA-GEO-PLAN.md wave G3 added
2, "Database change audit" below) — proves the trigger fires end to
...
**Wave G3's 2 additions**: `aCameraPoseUpdateProducesADbAuditLogRowNamingTheChangedColumn` saves a
`CameraPose` through `JpaCameraPoseRepository`, saves it again with a different `yawDegrees`, and asserts
the resulting UPDATE row's `changedColumns` contains `"yaw_degrees"` — proving `trg_audit_camera_poses`
actually fires end to end, not just that the migration declared it. `aTrackTrailInsertProducesNoDbAuditLogRow`
saves a `TrackPoint` through `JpaTrackTrailRepository` and asserts a native
`select count(*) from db_audit_log where table_name = 'projected_track_points'` stays `0` — proving the
exclusion is real, not merely undocumented.
...
`@Nested CameraPoseRepositoryTests` (6, docs/plans/done/FIXED-CAMERA-GEO-PLAN.md decision D4, wave G3) —
unknown asset → empty `Optional`; a pose with a `targetLayerId` and `rmsErrorPixels` round trips exactly,
including altitude; a manual pose with neither round trips both as `null`; `save` is an upsert leaving
exactly one `camera_poses` row per asset (a second save with a different `yawDegrees` replaces the first
in place, same id); `findAll` returns every saved pose; `deleteByAssetId` is idempotent (removes the row,
a second call on an already-deleted asset doesn't throw).

`@Nested TrackTrailRepositoryTests` (8, docs/plans/done/FIXED-CAMERA-GEO-PLAN.md decision D3/§7, wave G3) —
an unknown `(assetId, trackId)` gives an empty `findLatest` and an empty `findByTrack` list; a saved point
round trips exactly, including altitude; `findByTrack` returns points **oldest-to-newest regardless of
insert order** (points saved out of `capturedAt` order still sort correctly — proving the query orders by
the domain timestamp, not insertion sequence); `findLatest` returns the most recently *captured* point,
same distinction; `trimToMostRecent` keeps only the newest N points for a track (older ones are gone,
newer ones survive); `trimToMostRecent` is a no-op when the track is already at or under the cap (safe to
call every tick, per this wave's own non-negotiable); `trimToMostRecent` only affects the named track (a
sibling track's points are untouched — the `(asset_id, track_id)` scoping in the bulk-delete query is
load-bearing, not just the id); `deleteOlderThan` removes points across **every** track captured before
the cutoff, leaving newer points from every track in place.

`v22MigrationCreatesTheFixedCameraGeoTablesOnTopOfV1ThroughV21` (docs/plans/done/FIXED-CAMERA-GEO-PLAN.md
§7, wave G3) — same `information_schema.columns` shape as the V19-V21 schema tests above:
`camera_poses.asset_id` is `NOT NULL` (the primary key), `camera_poses.target_layer_id` stays nullable;
...
`@Nested TrackCorrectionRepositoryTests` (9, docs/plans/done/VISUAL-GEO-V2-PLAN.md §3.5/§3.7, wave H5) —
...
`DevAccountSeedMigrationTest` (5, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W1) — its own `@Testcontainers`
...
`UpgradePathMigrationTest` (4, docs/plans/done/POSTGRES-ONLY-CONTEXT.md upgrade path) — sibling to
...
`@Nested ConnectionPoolTests` inside `PostgresDockerIntegrationTest` (2, docs/plans/done/SCALE-100-PLAN.md
...
`PersistencePoolSettingsTest` (7, docs/plans/done/SCALE-100-PLAN.md S3, docker-free) — `defaults()` matches
...
`ClosingDatasourceConnectionProviderTest` (2, docs/plans/done/SCALE-100-PLAN.md S3, docker-free) — hand-rolled
...
148 tests total (up from 137, docs/plans/done/SCALE-100-PLAN.md S3: new `PersistencePoolSettingsTest` (7) +
...
137 tests total (up from 133, docs/plans/done/POSTGRES-ONLY-CONTEXT.md upgrade path: new
...
133 tests total (up from 128, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W1: new
`DevAccountSeedMigrationTest` = +5), run against a real `postgres:16` Testcontainers instance, docker
reachable in this environment — every new case actually ran, none skipped.

128 tests total (up from 115, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W3: the two ports with no Postgres
...
- **An applied migration is frozen byte-for-byte — comments included — so repo-wide text rewrites must exclude `db/migration` and `db/seed/dev`.** Flyway's checksum is a CRC32 over every line of the file, with no comment or whitespace tolerance, so editing a `--` header in a migration that has already run breaks boot with `FlywayValidateException: Migration checksum mismatch` on every database that ever applied it — the schema is untouched and the change is semantically empty, but the app will not start. This has already happened once: the `docs/plans/active/` → `docs/plans/done/` doc reorganisation rewrote plan-doc citations inside V13/V14/V15/V22/V23 and `V90001__dev_accounts.sql`, and every one of those files had to be restored to its shipped bytes. Consequence to live with, deliberately: **the plan-doc paths cited in those migrations' comments still read `docs/plans/active/...` and no longer resolve** — do not "fix" them; the plan is findable through docs/plans/README.md, which is the plan-status authority, and a migration comment is a historical record of what was true when it ran. The only legitimate way to change an applied migration's text is `flyway repair` on **every** deployed database (this module exposes no repair path — `PersistenceUnit#start` only calls `migrate()`), which is why the in-place edit of V23 in wave H8 was justified explicitly by "not yet shipped anywhere" (see Schema above) rather than treated as routine.
...
docs/plans/done/STREAM-STATE-PLAN.md **wave S5 done** (persistence half): `JpaAssetUsageRepository#findByStream(StreamId)` — the reverse of R-a2's link, reading the column that wave already added. **No migration in this wave**: the plan's own §2.6 pins it, and the column has existed since V4. Covered by two new `PostgresDockerIntegrationTest$AssetUsageRepositoryTests` cases (a stream's usage round-trips; an unknown stream and a `stream_id IS NULL` row both stay empty rather than matching) — **15/15** in that nested class, module **183/183** against a real Postgres 16 container.
...
- **`AuditTrailPort` now has a JPA implementation** (`JpaAuditTrail`, docs/plans/done/POSTGRES-ONLY-CONTEXT.md W3 — see that
...
## docs/plans/done/POSTGRES-ONLY-CONTEXT.md W1 done (the auth fix, standalone)
...
## docs/plans/done/POSTGRES-ONLY-CONTEXT.md W3 done (the two missing adapters)
...
later wave (docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2) deletes every `InMemory*` class and cannot delete these
...
(docs/plans/done/POSTGRES-ONLY-CONTEXT.md W1, which landed concurrently in the same working tree while this task ran:
...
## docs/plans/done/POSTGRES-ONLY-CONTEXT.md upgrade path done (fixing W1 for a database that already ran `AuthSeedRunner`)
...
## docs/plans/done/POSTGRES-ONLY-CONTEXT.md W2b done (JpaAuditTrail/JpaDetectionEventRepository wired; documentation catch-up — no source change in this module)
...
## docs/plans/done/SCALE-100-PLAN.md S3 done (a real HikariCP connection pool, shared with Flyway)
...
## docs/plans/done/SCALE-100-PLAN.md S4 done (telemetry write path — batching)
...
## docs/plans/done/FIXED-CAMERA-GEO-PLAN.md wave G3 done (fixed-camera geolocation, persistence half)

Gives `CameraPoseRepositoryPort`/`TrackTrailRepositoryPort` (`contexts/vision-map`, wave G2) their
Postgres implementations — see the entity/repository/mapper bullets above and the `V22__fixed_camera_geo.sql`
schema bullet for the full shape. New: `V22__fixed_camera_geo.sql` (two tables — `camera_poses`, one
calibrated pose per fixed asset, control-plane, upserted, **audited**; `projected_track_points`, the
decimated ground-track breadcrumb trail a projected detection leaves behind, append-only, high-volume,
**excluded** from `db_audit_log`, pruned by `trimToMostRecent`/`deleteOlderThan`), `CameraPoseEntity`/
`TrackPointEntity` (`entity` package), `CameraPoseMapper`/`TrackPointMapper` (`mapper` package),
`JpaCameraPoseRepository`/`JpaTrackTrailRepository` (`repository` package), both registered in
`PersistenceUnit`. Both `DbAuditLogCoverageTests` lists updated (`camera_poses` → `AUDITED_TABLES`,
`projected_track_points` → `EXCLUDED_TABLES`) — not optional bookkeeping, since that test reads the
live schema and fails loudly on an unclassified table.

**Base branch was stale.** This worktree's `feat/fixed-camera-geo` was pinned to a pre-reorg commit
(`331a6a77`), predating the module-layout reorg this wave's own file paths assume. Corrected before any
product code was written: `git checkout -b feat/fixed-camera-geo-g3 feat/fixed-camera-geo` from that
same tip (`188deb9a`), then confirmed `storage/persistence/` and the two G2 port files
(`CameraPoseRepositoryPort`/`TrackTrailRepositoryPort`) existed at that tip before proceeding — they did.

**Migration numbering confirmed, not assumed.** The plan's own text claims "V22 is next as of master
`e6f1d18e`"; this worktree's own `db/migration` directory was listed and sorted independently
(`ls ... | sort -V | tail -20`) and V21 was indeed the highest migration present, so `V22__fixed_camera_geo.sql`
was used as-is — no renumbering collision, unlike the hazard the plan's own brief warned might exist on
a concurrent branch.

**Gap in §7 as written, resolved rather than silently worked around**: the plan's own table sketch for
both `camera_poses` and `projected_track_points` lists only `latitude, longitude` for position, omitting
altitude — but `CameraPose#position()`/`TrackPoint#position()` are both kernel `GeoPosition` values whose
`altitudeMeters` is a real (if currently projection-unused) nullable component. Omitting the column would
have silently discarded that value on every save/load round trip, breaking the very round-trip contract
this wave's own exit criteria require. Both tables therefore carry a nullable `altitude_meters` column
not in the plan's own sketch — flagged here per this wave's brief rather than fixed in silence. Nothing
else in §7/D3/D4 was wrong or under-specified as written.

`./mvnw -B -pl storage/persistence -am test`: **205/205 green** (up from 188, `Skipped: 0`, docker
reachable — measured directly across three consecutive **foreground** runs of the same command, each a
clean `BUILD SUCCESS`, grepped with `-E "^\[(INFO|ERROR)\] Tests run:|BUILD"` to confirm no `[ERROR]`
line and a single `adapter-persistence` "Results:" block per run, not a reactor-wide aggregate). New:
`CameraPoseRepositoryTests` (6) + `TrackTrailRepositoryTests` (8) + 2 new `DbAuditLogRepositoryTests`
cases (a pose update names its changed column, a trail insert leaves no `db_audit_log` row) + 1 new
top-level schema test (`v22MigrationCreatesTheFixedCameraGeoTablesOnTopOfV1ThroughV21`) = +17, exactly
205 − 188. See "Tests" above for what each proves.

**Exit criteria, verified directly, not assumed from green**: `DbAuditLogCoverageTests` passes with
both new tables classified (`everyPublicBaseTableIsEitherAuditedOrExplicitlyExcluded`/
`everyAuditedTableCarriesExactlyTheAuditTriggerAndNoExcludedTableDoes`, both green against the live
schema); a `camera_poses` UPDATE produces a `db_audit_log` row naming the changed column
(`aCameraPoseUpdateProducesADbAuditLogRowNamingTheChangedColumn`); a `projected_track_points` INSERT
produces none (`aTrackTrailInsertProducesNoDbAuditLogRow`); `save`-then-`save` on the same `assetId`
leaves exactly one `camera_poses` row (`saveIsAnUpsertLeavingExactlyOneRowPerAsset`); `findByTrack`
returns oldest→newest regardless of insert order
(`findByTrackReturnsPointsOldestToNewestRegardlessOfInsertOrder`); `trimToMostRecent` keeps the newest N
(`trimToMostRecentKeepsOnlyTheNewestPoints`), is a safe no-op under the cap
(`trimToMostRecentIsANoOpWhenAlreadyAtOrUnderTheCap`), and never touches a sibling track
(`trimToMostRecentOnlyAffectsTheNamedTrack`) — all three needed since the brief calls out
"safe to call every tick" as a non-negotiable, not a nice-to-have.

**Deviations from the brief**: none that change behavior — the altitude-column addition above is the one
place this wave's output differs from §7's literal text, and it is called out rather than silently done.
`audit_row_change()` was not redefined, only attached (one new `CREATE TRIGGER`, reusing V21's function).
No magic numbers introduced: retention/point caps (`maxPoints` for `trimToMostRecent`, `cutoff` for
`deleteOlderThan`) are caller-supplied arguments, not constants in this module.
...
## Wave H8 (docs/plans/done/VISUAL-GEO-V2-PLAN.md §9.11 defect 4, §9.12) — done 2026-08-20
...
## docs/plans/active/CONTROLLER-SETUP-CONTEXT.md C4 done (saved controller layouts, persistence half)

`V24__control_profiles.sql`, `ControlProfileEntity`, `ControlProfileMapper`, `JpaControlProfileRepository`, plus the one-line `PersistenceUnit` registration. Two decisions worth keeping visible:

- **The uniqueness invariant lives in the database, not only in the repository.** `ControlProfileRepositoryPort` promises at most one active profile per `(owner, vehicle kind)`; `uq_control_profiles_active` is a partial unique index that makes that true even if a future caller forgets. `activate` therefore clears-then-sets inside one transaction — the other order trips the index.
