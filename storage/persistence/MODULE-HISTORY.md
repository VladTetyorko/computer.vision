...
## docs/plans/active/LINK-PAIRING-PLAN.md — wave L2 — a vehicle's persisted identity

`JpaPairingRepository`/`PairingEntity`/`PairingMapper` implementing the new `PairingRepositoryPort`
(§3.3): `merge` upsert by `PairingId`, `deleteById` a real **hard** delete (⚠ accepted deviation —
a forgotten pairing's sysid/key must stop being valid immediately, unlike this module's usual
soft-delete convention), `PairingMapper` round-tripping `VehicleKey`'s 32 raw bytes through
`vehicle_key BYTEA` unchanged. `V37__pairing.sql` adds the `pairings` table with unique indexes on
`device_id` (≤1 pairing per device) and `sysid` (the safety net behind `DefaultPairingService`'s own
collision check), audited via `trg_audit_pairings` on the same "operator-authored,
accountability-relevant, bounded write volume" reasoning as `control_profiles`/`vehicle_profiles`.

Covered by a standalone `repository.JpaPairingRepositoryTest` rather than one more section of
`PostgresDockerIntegrationTest` — the first exception to that monolith, deliberately, so this file's
own scope stayed disjoint from other concurrent work on that shared 5000+-line file; its own
`PersistenceUnit.start(...)` call migrates the schema independently (a second container, not a
shared fixture), and it asserts the vehicle key's 32 bytes round-trip byte-for-byte through `bytea`,
not merely via `VehicleKey#equals`.

This wave also fixed a latent bug the L2 domain change exposed in the existing monolith:
`DiscoveryCandidate#identityKeyFor` became sysid-first, address-blind once a sysid is known, so
several `PostgresDockerIntegrationTest#DiscoveryCandidateRepositoryTests#withStream` fixtures that
had shared one hardcoded sysid (`"7"`) across distinct addresses started colliding against the real
`uq_discovery_candidates_identity_key` unique index — three previously-green tests began failing
with `ConstraintViolation`. Fixed by giving every call a distinct sysid instead of adding cleanup
between methods.

`./mvnw -B -pl storage/persistence test` — **291** tests, up from 283 immediately before this wave
(+5 for the new `JpaPairingRepositoryTest`; the sysid-collision fix changed no test count).
