...
Build from the **repository root**, not from `cv/cv-service/` - the Dockerfile
does `COPY proto/ ./proto/` before `COPY cv/cv-service/ ./cv-service/`:

```bash
cd ../..    # repo root
docker build -f cv/cv-service/Dockerfile -t cv-service .
