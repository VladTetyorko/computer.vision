...
class ReferenceIndexStats:
    ...
    index_bytes: int
    # H4 extension (§4.2 G-e / §4.4 Change 2 G-a): count of individually never-accept tiles --
    # `cv_service.geo.calibrate.count_never_accept_cells`, computed once at build time. `0` on
    # regions built before this field existed (json read defaults it, see `read_index_json`).
    never_accept_cells: int = 0
...
def write_index_json(
    ...
    payload = {
        ...
        "neverAcceptCells": stats.never_accept_cells,
...
def read_index_json(region_dir: Path) -> Optional[RegionIndexMeta]:
    ...
    try:
        ...
        stats = ReferenceIndexStats(
            ...
            # Absent on regions built before this field existed -- defaults to 0 (neutral: "no
            # cell individually flagged", matching a pre-H4 region's own never-computed state).
            never_accept_cells=int(payload.get("neverAcceptCells", 0)),
        )
