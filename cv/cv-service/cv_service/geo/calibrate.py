...
# --- per-cell calibration (VISUAL-GEO-V2-PLAN.md §4.2 G-e / §4.4 Change 2 G-a, H4 extension) ----
#
# H0/H0c's own bake-off scripts (`spikes/geo/run_bakeoff.py` etc.) never built true per-cell
# calibration -- they computed ONE region-wide `is_never_accept` bool and reused it as every
# candidate's `cell_is_never_accept`. The plan's harvest manifest (§1.3) calls for more: calibrate.py
# is "extended with the per-cell calibration record §4.4 G-a needs", and D5 itself says
# "per-cell self-calibrated accept_similarity/accept_margin".
#
# This is the honest middle ground: it does not invent a second per-tile calibration sweep (no
# per-tile holdout set exists, and building one was judged out of this wave's scope -- a whole
# region's calibration already costs one holdout sweep). Instead it reuses data every region
# already computes at build time for a different purpose --
# `leave_one_out_distinctiveness` (Slice A, §13.3 item 1) -- and applies a simple, provable bound:
# a tile whose own leave-one-out margin (the best accept-margin ANY query centered exactly on
# that tile could ever produce, against the rest of the region) is below the region's calibrated
# `accept_margin` can never legitimately clear the region's own gate. Such a tile is therefore
# never-accept individually, even when the region as a whole is not -- exactly what G-e ("the
# winning cell's own accept_similarity is not the never-accept sentinel") and the wire's
# `ReferenceIndexStats.never_accept_cells` (a genuine per-tile count, not a region-wide 0/all
# switch) are asking for.
#
# `distinctiveness=None` (a tile from a region built before Slice A, or the pre-Slice-A on-disk
# format) is treated as neutral -- consumers cannot rule it out, so it defers entirely to the
# region-wide flag, the same "None = neutral" contract `tiles.json`/`ReferenceTileMeta` already
# document.


def cell_is_never_accept(
    distinctiveness: Optional[float], *, region_is_never_accept: bool, accept_margin: float
) -> bool:
    """True when this specific tile can never legitimately clear the region's own calibrated
    gate -- either because the region itself is never-accept, or because this tile's own
    leave-one-out margin is below the region's `accept_margin` (see module-section docstring)."""
    if region_is_never_accept:
        return True
    if distinctiveness is None:
        return False
    return distinctiveness < accept_margin


def count_never_accept_cells(
    distinctiveness_by_tile: Sequence[Optional[float]],
    *,
    region_is_never_accept: bool,
    accept_margin: float,
) -> int:
    """`ReferenceIndexStats.never_accept_cells` -- how many of a region's indexed tiles are
    individually never-accept, computed once at build time (`orchestrator.py`) over every
    tile's own `distinctiveness`."""
    return sum(
        1
        for d in distinctiveness_by_tile
        if cell_is_never_accept(d, region_is_never_accept=region_is_never_accept, accept_margin=accept_margin)
    )
