...
encode+calibrate pass can run for minutes, same as a multi-epoch train, and must stay responsive
to client cancellation).

Ties together `cv_service.geo.{pack,index,calibrate,encoder}` into one job: scan a landed region's
tiles, hold out a fraction for self-calibration, encode the rest into a
...
**Carries H0c's own two production fixes verbatim (§9.9 amendment 4 requires both survive the
port unchanged)**:
- **Defect 1 (index coverage)** -- every readable tile ends up indexed, not just the
  `1 - holdout_fraction` reference split (the holdout split exists ONLY to measure
  `accept_similarity`/`accept_margin` honestly; it must never make a tile unlocalizable by a live
  query -- see the inline comment above `holdout_tiles_meta` below for the full history).
- **Defect 2 (rectification never applied)** does NOT apply to this module -- it is a
  `LocalizeStream`/query-time defect (`cv_service/geo/localize.py` IPM-rectifies the QUERY frame
  before matching); reference tiles are already-nadir orthophotos with no oblique geometry to
  rectify, so region building has nothing to fix here.

**H4 addition**: computes `ReferenceIndexStats.never_accept_cells`
(`calibrate.count_never_accept_cells`, §4.2 G-e / §4.4 Change 2 G-a) over the full persisted set's
own leave-one-out distinctiveness, alongside the region-wide calibration this module already ran.

Needs `cv2` (tile image decode) + `numpy` -- the `geo` extra, same "extra-gated" discipline as
`cv_service/inference/detector.py` for the `cv` extra. `cv_service/grpc/servicers.py` only imports
this module lazily, inside `BuildReferenceIndex`, never at module scope.
"""
...
from cv_service.geo import calibrate, index, pack
...
def build_region_index(
    ...
    *,
    ...
    # H4 (§4.2 G-e / §4.4 Change 2 G-a): per-cell never-accept count over the FULL persisted set
    # (same set `distinctiveness` above was computed over) -- a region-wide never-accept
    # (`calibrate.is_never_accept`) makes every tile never-accept; otherwise a tile is
    # individually never-accept iff its own leave-one-out margin can't clear the region's
    # calibrated `accept_margin` (see `calibrate.py`'s per-cell-calibration section docstring).
    region_is_never_accept = calibrate.is_never_accept(
        calibration.accept_similarity, calibration.holdout_recall_at_1
    )
    never_accept_cells = calibrate.count_never_accept_cells(
        [m.distinctiveness for m in all_tiles_meta],
    ...
    stats = index.ReferenceIndexStats(
        ...
        never_accept_cells=never_accept_cells,
    )
