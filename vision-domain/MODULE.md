...
- `record PipelineConfig(ModelRef model, double confidenceThreshold, int inferenceFps, int maxInFlightInferences, boolean overlayTelemetry, Set<String> labelFilter, EventRuleConfig eventRule)` — inferenceFps/maxInFlightInferences>0; `eventRule` non-null (docs/MVP2-PLAN.md §E, E-a); a 6-arg convenience ctor defaults `eventRule=EventRuleConfig.defaults()` (same "N-1-arg convenience ctor" idiom as `Asset`/`AssetUsage`); `static defaults()` = yolo/latest, 0.4, 10fps, 2 in-flight, telemetry on, no filter, `EventRuleConfig.defaults()` (10fps since docs/CYCLES-PLAN.md §12, CP-c — was 5fps)
