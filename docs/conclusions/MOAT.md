...
Status: **thinking document for review (2026-08-09)**. Companion to docs/conclusions/FEATURE-MATRIX.md (which
ranks features by effort/value) — this one asks the prior question: *which of them are ours alone?*
Companion to docs/conclusions/ANY-DRONE-PLAN.md (the adoption funnel).
...
(docs/plans/done/CV-TRAINING-PLAN.md, done; open-vocabulary YOLOE so day one isn't empty).
...
verify-and-promote to the common picture, drawings, scoped live updates (docs/plans/done/MAP-REWORK-PLAN.md,
done). Plus remote piloting over IP with a physical transmitter (docs/plans/active/RC-CONTROL-PLAN.md) and
...
docs/conclusions/ANY-DRONE-PLAN.md. The person who already owns hardware — a DIY quad, an INAV wing, a
