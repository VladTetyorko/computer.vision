...
Implements **S1** of docs/main/TWO-TARGETS-PLAN.md ("the one
...
> **Companion doc: [docs/extracts/TRACKING-ORCHESTRATION.md](../../extracts/TRACKING-ORCHESTRATION.md)** — component
...
| PATCH surface | `PATCH /api/streams/{id}/config` — confidence, fps, labelFilter, detectionEnabled, model (docs/plans/done/CV-CONTROL-PLAN.md §3) | one more optional object, `tracking` — including the target lock. **No new endpoint for click-to-follow** |
...
**(b) is the real onboard story.** It maps to **H4** in docs/main/TWO-TARGETS-PLAN.md (companion
...
**docs/main/TWO-TARGETS-PLAN.md §S2 — this plan supplies its input, and nothing more.**
...
The existing endpoint, contract and status codes (docs/plans/done/CV-CONTROL-PLAN.md §3) are unchanged. One
...
frozen decision `GET /api/cv/models` already made (docs/plans/done/CV-CONTROL-PLAN.md §D) — the roster changes
...
reasoning docs/plans/done/CV-CONTROL-PLAN.md §C used for `detectionEnabled`.

### H. Stream→worker affinity becomes load-bearing

docs/plans/active/CV-SCALE-PLAN.md §S4 already commits to **sticky-least-loaded** routing —
...
- **Geolocation projection of tracks (S2)** — tracks are its input; the projection, the camera-pose
  attributes and the calibration flow are docs/main/TWO-TARGETS-PLAN.md §S2, not this plan.
...
| **R6** | **Stream→worker affinity becomes a correctness requirement**, not an optimization | Medium | Recorded as a hard constraint on docs/plans/active/CV-SCALE-PLAN.md §S4's `PooledDetectionPort`; that wave must carry a test that a stream never migrates workers mid-flight. Not built here |
...
tier" (docs/main/TWO-TARGETS-PLAN.md H4) a bandwidth budget rather than a wish.
