...
2. **New in-flight work (docs/plans/done/REALTIME-PLAN.md Phase R-b, uncommitted as of this analysis) is
...
└── live/                                      ← NEW, empty until docs/plans/done/REALTIME-PLAN.md Phase R-c
...
shows exactly what's mid-flight (docs/plans/done/REALTIME-PLAN.md Phases R-a/R-b, and a backend-only R-c
...
## 4. Interaction with docs/plans/done/REALTIME-PLAN.md (so this plan doesn't fight it)
...
- **Phase R-c** (`LiveStore`, the single `EventSource`/SSE connection; `FleetStore`/
  ...
  likely to start (Phase C, scheduled well before R-c per docs/plans/done/REALTIME-PLAN.md's own phase
