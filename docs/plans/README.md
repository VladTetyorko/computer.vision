...
|---|---|---|---|---|
| AF | **Asset flows & control** — same-day research corpus (R1 code truth: 15 gaps · R2 pilot journey · R3 prior-spec mining · R4 industry patterns) + O1 synthesis + **ranked proposal awaiting user picks**. Safety tier S1–S6: grounding doesn't gate arm/disarm, dual-command (=row 10), cockpit-vs-fleet battery verdicts disagree, no link-lost/battery-low notification (min slice of row 12), readiness ignores live telemetry, mediamtx open (4th independent flag). Seven decisions D1–D7 incl. the missions 4-way contradiction | [ASSET-FLOWS-CONTEXT](active/ASSET-FLOWS-CONTEXT.md) → [P-PROPOSAL](active/asset-flows/P-PROPOSAL.md) | picks pending | Nothing scheduled until the user picks; candidates attach to rows 5/6/10/12 rather than duplicate them |
