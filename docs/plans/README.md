...
| W | **Warehouse + sidebar** — **MERGED to master 2026-08-30**: `Identity`/`Custody`/`InventoryState` beside `LifecycleState`, `MaintenanceRecord` grounds a vehicle (readiness NO-GO, engage refuses), passive equipment via `category.connected`, V28; one Inventory page (Vehicles · Equipment · Links · Categories, CSV export), five-group rail 25→18 with 0 stubs, wizard Identify→fit-out→Prove→Register→Hand over, Maintenance (one-call `GET /api/maintenance`) + Crew pages, Firmware/Hours columns | [WAREHOUSE-UX-PLAN](active/WAREHOUSE-UX-PLAN.md) | M–L | Open: Reports' attention list, `documents` table. Its §3.3 page/verb design and its half-shipped D3 (issue ⇒ custody **and** assignment) are superseded by row **INV** below |
| INV | **Inventory rework** — `/assets` shaped by who is looking. Visibility audit (2026-09-06): **server scoping correct** on every inventory read/write; the **web had drifted** — manager verbs rendered for pilots/viewers, `issue` set custody only (a pilot handed a vehicle could not see it), UUIDs where names belong (`GET /api/users` is empty for an ASSIGNED_ASSETS scope), `loadAll` N+1. Design: manager = table whose KPI strip *is* the view switcher (Needs attention · In field · Issued · In stock · Maintenance) + §6-anatomy drawer with "Why not ready"; pilot/crew = **My vehicles** cards; viewer = read-only table; one state × authority verb matrix drives kebab and cards. Analog survey of nine systems in `inventory-rework/R1-analogs.md`. **W0 docs `f6382db4` · W1 `49a6411b` (identity `HandoverService`: issue = custody + idempotent PILOT assignment with compensation; `custodianName`, pilot `displayName`, `deviceCount` on the wire) · W3 `5f98f834` (authority-aware `vehicleRowActions(row, actor)`, details on selection, 5 flat requests) BUILT 2026-09-06** on `feat/inventory-rework`; W4 views/drawer/dialog + W5 cards + W6 live matrix in flight | [INVENTORY-REWORK-CONTEXT](active/INVENTORY-REWORK-CONTEXT.md) → [INVENTORY-REWORK-PLAN](active/INVENTORY-REWORK-PLAN.md) | M | Owner asked 2026-09-06 to rework the page around what each user clicks in each situation and to double-check visibility. **D4 (pilot may Report issue) / D5 (custodian may Return) widen authorization and wait on the owner — W2 does not start before that.** `AssetInventoryController` now at 6 ctor params (debt: split the maintenance endpoints off) |
