...
| — | **Operator control** — TX12/EdgeTX, generalized over ArduPilot/INAV/Betaflight | [OPERATOR-CONTROL-CONTEXT](active/OPERATOR-CONTROL-CONTEXT.md) — decisions D1–D13, **no plan written yet** | M | Carries two verified latent bugs: **G13** chan 9–16 release sentinel is `65534`, not `0` · **G14** we never check `SYSID_MYGCS`, so overrides can be dropped in silence |
