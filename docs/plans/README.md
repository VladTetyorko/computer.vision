...
| W | **Warehouse + sidebar** — an inventory layer on `Asset` (identity, custody ≠ authorization, `InventoryState` beside `LifecycleState`, maintenance records that ground a vehicle in readiness, passive equipment via `category.connected`), one Inventory page with tabs instead of five list pages, a 5-group rail with 0 stubs (25→15 entries), wizard that starts with the thing and ends with a hand-over | [WAREHOUSE-UX-PLAN](active/WAREHOUSE-UX-PLAN.md) — proposal, waves W1–W7 | M–L | Every question about the fleet became its own page because there is no inventory model underneath; W1 (rail) is a day and fixes the "overwhelming" complaint alone; W3 is the dedicated `V28` task the persistence freeze asks for; closes row 6 in passing |
