...
Status: K/R **built** 9062f82d — keyboard is a third `RcSourceKind` (`core/rc/keyboard-rc-input.service.ts`), key legend under the pads, and readiness rows (`rc-relay` only, advisory, "Open preflight ›") under Take control; `RcSource#keyboard` is `{ optional: true }`-injected so the two pre-existing hand-built harnesses (`manual-control-client.spec.ts`, `control-action-dispatcher.spec.ts`) didn't need touching. M **built** 2997c53f — Mode-step datalist fed by `selectableModes` of streaming assets of the layout's kind, honest source hint, free text kept. **Cycle 2 merged to master 9f140700 (2026-08-28).**
