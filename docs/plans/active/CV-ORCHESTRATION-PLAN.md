...
**As built in W9 (merged 2026-09-13, `83d3a791`)** — the `tracks:` envelope carries the whole `/tracks` snapshot, built once; sub-branch `feat/cv-orchestration-w9-tracks-topic` @ `1cc8fe7d`, five commits `f7b1fce8` (W9.0a) → `4e4293ee` (W9.0) → `1ff21c6f` (W9.1) → `781182c2` (W9.2) → `1cc8fe7d` (W9.3 docs), reviewed by git and merged by Fable. Merged-tree verification (task worktree, foreground, `clean install` then `test`): perception 886 / cv-grpc 195 / persistence 286 / vision-api 1122 / vision-app 357 with Docker; `npm run test:ci` 209 files / 4062 tests; both tsconfigs type-check clean.

| Design (E24 / brief) | As built | Why |
|---|---|---|
| `TrackingStats` / `PipelineLatency` / `DetectionRate` live in `application.pipeline` | moved to `domain.model` as a pure package move (`git show -M`: two of the three files differ only in their package line); the windows (`TrackingStatsWindow`, `PipelineLatencyWindow`, `DetectionRateWindow`) stay in `application.pipeline`; `TrackingStats#zeroedStates()` package-private → public because its one caller is now another package; two javadoc `{@link}`s to application types became `{@code}` | E27: a domain port cannot carry application types (ArchUnit `domainDependsOnlyOnDomainAndJava`); no per-class exception was added, the rule stays generic and is green at 14/14 |
| `TracksSnapshot` "assembled at `onDetectionResult`" | `domain.model.TracksSnapshot(streamId, tracks, Optional stats, Optional latency, Optional rate, Optional detectionState, Optional follow, objects)` — the compact constructor rejects every null and copies both lists; `empty(streamId)` factory; `StreamPipeline#tracksSnapshot()` is the single assembly point, also exposed as `StreamService#tracksSnapshot(StreamId)` | one read model for both transports |
| capture point "right where the old `world.objects()` capture was" | captured at the **end** of the `if (live)` block of `onDetectionResult`, after `trackingStats.accept` / `rateController.observeDetections`, and published only inside that block | newest-data-wins (CLAUDE.md rule 7): the pushed stats/latency/rate must not lag the frame that produced them |
| `DetectionLiveUpdatePort#publishDetections(AssetId, DetectionResult, List<WorldObject>)` | third parameter widened to `TracksSnapshot` ("never null" in the port contract; `LiveUpdateRegistry` `requireNonNull`s it); the fakes changed at their call sites, no overload (`NoopLiveUpdatePublisher`, `LiveAssetScopingTest`, `LiveUpdateRegistryTest`) | rule 10 |
| one assembly `StreamTracksResponse.from(snapshot, now)` | built; `StreamController#tracks` collapsed to the visibility check + `tracksSnapshot(id).orElse(TracksSnapshot.empty(id))` + `from(…)`; `LiveUpdateRegistry#flushPending` builds the `tracks:<assetId>` envelope from the same call. The null rules are the old controller's, line for line: stats without `lastDetectorReason` → null, latency with 0 samples → null, rate with 0 due → null, `lockedTrackId` only for HOLDING/COASTING. `StreamControllerTest`'s `/tracks` HTTP assertions are byte-identical; only its Mockito arrange lines collapsed from five stubs to one | "one assembly, two transports" — verified in review against `60f74ee3`'s controller, not the report |
| wire pinned | `StreamTracksResponseWireContractTest` (Java) and `stream-tracks.wire.contract.spec.ts` (TS) pin the committed fixture `core/api/__fixtures__/stream-tracks.wire.json` | both sides of the wire agree on the same bytes |
| — | payload per `tracks:` envelope 243 → 1,596 bytes (measured on the fixture), now at frame cadence instead of the 2 s poll cadence | to be weighed at the next capacity decision (SCALE-100 Band B was never load-tested) |
| `DetectionsStore` "drops the `GET …/tracks` poll" | `LiveStore.tracksFor(assetId)` / `worldObjectsFor` split off the same ref-counted subscription; `DetectionsStore.tracks` reads the live signal when the transport is live and an asset id is in scope, the 2 s poll otherwise; `followTracks` keeps its signature; three new spec cases (never polls once live with an asset id; still polls without one; poll → live flip mid-session) | `LivePage` / `WallTile` mount without an asset id, so the poll stays as the fallback rather than dead code |
| build | the package move leaves stale `.class` files: without `clean`, three unchanged perception test files false-failed on the sub-branch; the merged-tree verification therefore ran `clean install` first | lesson: a package move needs `clean` on every scoped run |
| — | untouched by design: the `worldObjects` subscription lifecycle (W3.1's `track()` / `teardownTracking()` pair) and every W7-scoped file; open by rule: W6 cold ledger, W-legacy (E20) | scope |
...
| **W9** tracks poll retirement — **DONE** 2026-09-13 (E25/E27; `83d3a791`, sub-branch `feat/cv-orchestration-w9-tracks-topic` @ `1cc8fe7d`, five commits `f7b1fce8` → `1cc8fe7d`; merged-tree perception 886 / cv-grpc 195 / persistence 286 / vision-api 1122 / vision-app 357 with Docker, `test:ci` 209 files / 4062, both tsc clean) | `tracks:` payload = the whole `StreamTracksResponse` snapshot; one assembly `StreamTracksResponse.from(TracksSnapshot, Instant)` serves both `GET …/tracks` and the SSE push; `DetectionsStore.tracks` reads the live topic whenever an asset id is in scope and polls otherwise | Sonnet | zero `GET …/tracks` requests while a viewer with an asset id is mounted (spec: `detections-store.spec.ts` "never polls once an assetId is in scope and LiveStore is open"); `LivePage`/`WallTile` (no asset id) keep polling by design; the wire is pinned by `stream-tracks.wire.json` on both sides |
