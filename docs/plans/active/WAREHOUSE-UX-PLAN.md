...
Status: **W1–W7 built on `feat/warehouse-ux`, unmerged** (2026-08-30; cut from master `59b879a5`). Ledger, handoffs and deviations in [WAREHOUSE-UX-CONTEXT.md](WAREHOUSE-UX-CONTEXT.md). Rail count landed at **18 / 10** (manager / pilot), not the 15 §3.1 estimated — Add vehicle, Crew, Maintenance and the four SYSTEM entries were under-counted there. Open after this cycle: a fleet-wide maintenance endpoint (the page is O(grounded assets) reads), Firmware/Hours columns have no producer yet, Reports' attention list was not re-homed, a `documents` table (E8). Read after [OPERATOR-UX-7-PLAN](OPERATOR-UX-7-PLAN.md) (page polish) and [SOURCE-ONBOARDING-CONTEXT](SOURCE-ONBOARDING-CONTEXT.md) (the add-a-vehicle wizard); this plan is the level above both — the *entity*, the *tables* and the *sidebar*.
