...
Status: **proposal, nothing built** (2026-08-29). Branch: none yet — cut `feat/warehouse-ux` from master when the first wave starts. Read after [OPERATOR-UX-7-PLAN](OPERATOR-UX-7-PLAN.md) (page polish) and [SOURCE-ONBOARDING-CONTEXT](SOURCE-ONBOARDING-CONTEXT.md) (the add-a-vehicle wizard); this plan is the level above both — the *entity*, the *tables* and the *sidebar*.
