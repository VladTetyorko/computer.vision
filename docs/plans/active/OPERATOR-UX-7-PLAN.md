...
Status: W1 (`659a7766`), W2 (`7f2891c8`), W3 (`8ad8b4e8`) + sweeps (`91d79dcb`, `c198a100`) built; **merged to master 2026-08-29** (154 files / 2958 web tests, prod build green). Facts learned: the events backlog lands one event at a time, so any 'seed on the first (non-empty) tick' read-set is wrong — unread is now gated by the bell's mount time exactly like `shouldToast` (persisted read ids stay for dropdown-open acknowledgements); a shared `file://` or `rtsp://` URI is not a conflict — only bound `udp://`/`tcp://` sockets are exclusive; `hasBeenProbed` reads the per-asset report, the fleet board needs its own `isNeverProbedRow`, so the board cannot tell 'never probed' from 'probed, all unknown'. Open: per-asset `getAsset` fan-out on the pre-flight board (N+1, precedented by `/assets`).
