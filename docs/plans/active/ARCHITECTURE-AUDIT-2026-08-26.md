...
| R1b | **merged** | `AssetUsage` still carried three convenience constructors R1 never reached, and R2 added a fourth. Collapsed to the one canonical constructor, every call site explicit. The census below is what it cost to find out this was not the last one. |
...
| R9 | **verified** | `./mvnw -B clean verify` on the branch tip: **36/36 modules SUCCESS**, 7:53 min, **4,156 Java tests** across 24 test-bearing modules plus **2,530** vitest specs, **0 failures, 0 errors, 2 skipped** — and both skips are the `@Disabled` `FfmpegVideoSourceLiveManualTest` and `WaybackTileSourceLiveManualTest`, the only two tests in the tree that dial the live network. `clean` is not optional here: a stale `V25` in `target/classes` from an unrelated branch made Flyway fail with "Found more than one migration with version 25" on an incremental run. Six merged wave branches and their worktrees pruned. |
