...
| H2 | adapter-builder | peer-gate identity: firmware's first-peer authority gate applies to `RC_CHANNELS_OVERRIDE` too (H1 firmware note) → the station MUST present ONE wire identity (same UDP socket/source endpoint) for telemetry listening and RC-override sending toward one vehicle, or the rover obeys one and silently ignores the other ("engaged but inert"). Verify whether station telemetry RX and control TX share a socket today (drone-link/mavlink + mavlink-core session/peer layer); fix station-side if split; tests prove one-socket identity; append precise firmware-owner requirement if any residue remains. **Owner has uncommitted changes in drone-link/mavlink-core + infra/rover-sim — build on top, never revert/stage them** | scoped `-pl` tests |
