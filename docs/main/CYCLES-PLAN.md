...
Companion to [ARCHITECTURE.md](../../ARCHITECTURE.md), [UX-DESIGN.md](UX-DESIGN.md), [WEB-PLAN.md](../plans/done/WEB-PLAN.md).
...
| C7 | backend | real YOLO inference + gRPC DetectionPort + outage resilience | [MVP1-PLAN.md](../plans/done/MVP1-PLAN.md) §C7 | ✅ done |
| C8 | UI-facing | overlay burn-in + detections endpoint + Live strip | [MVP1-PLAN.md](../plans/done/MVP1-PLAN.md) §C8 | ✅ done |
| C9 | demo | compose + demo script + E2E | [MVP1-PLAN.md](../plans/done/MVP1-PLAN.md) §C9 | ✅ done |
...
C7–C9 execute **[MVP1-PLAN.md](../plans/done/MVP1-PLAN.md)** — the priority target ("the friends demo": simultaneous multi-protocol sources + map + live CV) — complete. The pending UI cycles above (CT-b, CU-b, CD-b) and all post-MVP candidates are folded into **[MVP2-PLAN.md](../plans/done/MVP2-PLAN.md)** (persistence, WebRTC viewing, flight replay, MAVLink + V4L2 protocols, detection events).
...
6. **Client-side detection overlay (crisp boxes):** draw detection boxes as a vector overlay (canvas positioned over the `<video>`, boxes from `GET /api/streams/{id}/detections` polling) instead of relying only on the server burn-in — boxes stay pixel-perfect at any video bitrate and become hover/clickable (label + confidence). **Sync caveat:** HLS runs seconds behind live, so boxes must be matched by `capturedAt` against the player's live-edge latency estimate (hls.js `latency`), not painted immediately; tolerate ±1 frame-batch of slack. Server burn-in stays (it's what recordings/export see); a per-tile toggle "boxes: overlay/burned/off" defaults to overlay when detections are flowing. Backend prerequisite already exists (detections REST, CD-a `deviceId`); no server change.
