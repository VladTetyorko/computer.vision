 ...
 *
 * <p>{@link Frame#image} is backed by native memory that the grabber reuses
 ...
 * {@code FrameConverter}, not a shared dependency: per CLAUDE.md's
 * dependency rule, adapters must not depend on each other, so this small
 * conversion routine is reimplemented here rather than imported.
 *
 * <p>Package-private and stateless so it is unit-testable in isolation,
 * without any grabber/device I/O.
 */
final class V4l2FrameConverter {

    private V4l2FrameConverter() {
    }
